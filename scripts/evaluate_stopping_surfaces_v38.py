"""V38 translational STOP deceleration on the exposed surfaces bank."""
import argparse
import gzip
...
FREEZE=ROOT/"experiments/walking/stopping-surfaces-freeze-v38.json"
...
SOURCES = list(dict.fromkeys(SOURCES + ["scripts/evaluate_stopping_surfaces_v38.py", "microduck/stopping_ramp_v38.py", "experiments/walking/STOPPING-RAMP-v38.md", "tests/test_stopping_ramp_v38.py"]))
...
def main():
    ...
    p.add_argument("--candidate",choices=("baseline",),default="baseline")
    a=p.parse_args()
    ...
    walking_run="walking-20260906-v21" if a.candidate=="baseline" else "walking-20260906-v25"
    standing_run="standing-20260906-v25" if a.candidate=="paired" else "standing-20260906-v15"
    ...
    try:
        with gzip.open(a.output/"trajectory.jsonl.gz","wt") as stream:
            for session in sessions:
                ...
                world.command_ramp = StoppingRamp()
                full_rows = []
                            ...
                            try:
                                for i in range(round(window["duration_s"]*50)):
                                    ...
                                    if writer and i%2==0:
                                        ...
                                        draw.text((8,8),f'V38 | {name} | {row["session_time_s"]:.2f}s | {role}',fill="white")
        ...
        result={"schema":"microduck.terrain-endurance-result/v1","bank":a.bank,"controller_variant":"stopping-ramp-v38","candidate":a.candidate,"proof_class":"visible_development",
