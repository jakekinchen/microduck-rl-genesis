...
FREEZE = ROOT/'experiments/walking/exact-hull-freeze-v34.json'
...
def binding():
    ...
    value.update(schema='microduck.exact-hull-freeze/v34',
    ...
    for name in ['scripts/probe_exact_hull_v34.py', 'experiments/walking/EXACT-HULL-v34.md', 'microduck/exact_hull_env_v34.py', 'experiments/walking/collider_geometry_v34.py']:
...
def run(output, result):
    ...
    for index,(directory,session_id) in enumerate(CASES):
        ...
        item = dict(session_id=session_id,input_receipt=directory,steps=len(rows),physics_steps=len(history),
            ...
            native_plane_reproduced=True,genesis_variant='original-mpr-no-mesh-decimation',genesis_original_max_qpos_delta=observer_delta,comparisons={})
...
def main():
    ...
    result = dict(schema='microduck.exact-hull/v34',status='starting',cases=[],physical_calibration=False)
