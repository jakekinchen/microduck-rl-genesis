...
FREEZE = ROOT/'experiments/walking/unpruned-patch-freeze-v36.json'
...
def binding():
    ...
    value.update(schema='microduck.unpruned-patch-freeze/v36',
    ...
    for name in ['scripts/probe_unpruned_patch_v36.py', 'experiments/walking/UNPRUNED-PATCH-v36.md', 'microduck/unpruned_patch_env_v36.py', 'experiments/walking/collider_geometry_v34.py']:
...
def run(output, result):
    ...
    from microduck.unpruned_patch_env_v36 import UnprunedPatchWalkingEnv as PublicSurfaceWalkingEnv
    ...
    for index,(directory,session_id) in enumerate(CASES):
        ...
        item = dict(session_id=session_id,input_receipt=directory,steps=len(rows),physics_steps=len(history),
            ...
            native_plane_reproduced=True,genesis_variant='unpruned-exact-hull-gjk-patch',genesis_original_max_qpos_delta=observer_delta,comparisons={})
...
def main():
    ...
    result = dict(schema='microduck.unpruned-patch/v36',status='starting',cases=[],physical_calibration=False)
