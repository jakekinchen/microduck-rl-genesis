...
def main():
    ...
    applied = [[], []]
    rows = []
    initial_qpos = [env.robot.get_qpos()[0].cpu().numpy().copy(), c.data.qpos.copy()]
    try:
        for i, action in enumerate(inputs):
            ...
            for substep in range(4):
                ...
                gen_qpos = env.robot.get_qpos()[0].cpu().numpy().copy()
    ...
    report = {"schema": "microduck.cross-engine-fixed-action-diagnostic/v1",
        ...
        "state_observation": "raw generalized qpos, not one-step-old derived link transforms in compatibility mode",
    ...
    for name in ("scripts/audit_walking_solver_qstate.py", "microduck/walking_ground_env.py", "microduck/walking_contact_env.py", "microduck/walking_solver_env.py", "microduck/walking_tracking_env.py", "microduck/walking_posture_env.py", "microduck/walking_controlled_env.py", "microduck/walking_env.py", "microduck/velocity_env.py", "microduck/bam_actuator.py", "microduck/constants.py", "experiments/walking/world.py", "evaluator/core.py"):
