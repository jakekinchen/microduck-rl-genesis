"""Read complete V55 evidence and decide prerequisites without rerunning physics."""
import gzip
import hashlib
import json
...
from experiments.walking.yaw_v55 import (FREEZE,RECIPES,REPLAY,candidate,digest,
...
def verify_evaluation(folder,recipe,expected_count,session_bank):
    ...
    if recipe in ('control','yaw6','parent'):
...
def main():
    ...
    out=new_output('receipts/walking/20260912-v55-verification')
    ...
    parent_fresh,parent_parity=verify_evaluation(ROOT/'receipts/walking/20260912-v55-parent-fresh','parent',2,True)
    ...
    for recipe in RECIPES:
        ...
        expected=np.where(active,WEIGHTS[recipe]*np.abs(rewards[:,:,0]-rewards[:,:,1])/.20,0)
        ...
        for bank,count,is_session in [('flat',21,False),('repeated',42,False),('endurance',4,True),('surfaces',14,True),('fresh',2,True)]:
            path=ROOT/'receipts/walking'/f'20260912-v55-{recipe}-{bank}'
    ...
    for bank in ['flat','endurance','surfaces']:
        verify_sources(json.loads((ROOT/f'experiments/walking/yaw-{bank}-freeze-v55.json').read_text()))
    require_freeze()
