"""V27 bounded surface-curriculum refinement from source-bound V21 FINAL."""
import argparse
import copy
...
def main():
    ...
    p.add_argument("--role",choices=("walking",),required=True)
    ...
    from scripts.evaluate_public_curriculum_v27 import FREEZE, SOURCES
    ...
    record = {"schema": "microduck.walking-training/v1", "variant": "public-curriculum-v27",
    ...
    try:
        ...
        from microduck.public_surface_curriculum_v27 import ConservativeSurfaceWalkingEnv
        ...
        env_class=ConservativeSurfaceWalkingEnv
        ...
        def stop(signum, frame):
            raise KeyboardInterrupt(f"owned V27 training interrupted: {signum}")
        ...
        if a.num_envs==1024 and not (all(x>100 for x in record["final_surfaces"]["loaded_contact_samples_by_profile"][:3]) and record["final_surfaces"]["loaded_contact_samples_by_profile"][3]==0):
            raise ValueError("intended three-panel exposure failed or stress-only panel contaminated training")
