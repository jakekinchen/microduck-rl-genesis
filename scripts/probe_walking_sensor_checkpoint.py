...
def main():
    ...
    for motor,sensor in ((4,1),(0,0)):
        for case in (suite["cases"][1],suite["cases"][3]):
            ...
            world=ConsistentSensorWalkingWorld(ROOT/"receipts/laser-gait/20260905-v4-evaluation/policy.onnx",ROOT/".workspace/bam",motor_ticks=motor,sensor_ticks=sensor,yaw=case["yaw"],render=a.video)
    ...
    (a.output/"probe.json").write_text(json.dumps({"schema":"microduck.walking-intermediate-probe/v1",
        ...
        "inference":"Torch CPU; normalized actor, copied-current-state IMU; not ONNX acceptance","cases":reports},indent=2)+"\n")

    shutil.copy2(checkpoint,a.output/"source-checkpoint.pt")
    shutil.copy2(folder/"run.json",a.output/"training-at-probe.json")
    for name in list(record["source_sha256"])+["scripts/probe_walking_sensor_checkpoint.py","experiments/walking/sensor_world.py","experiments/walking/world.py","experiments/walking/posture.py","experiments/walking/metrics.py","experiments/walking/suite-v1.json","experiments/laser/gait.py"]:
