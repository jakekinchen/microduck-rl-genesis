...
FREEZE = ROOT/'experiments/walking/contact-patch-freeze-v32.json'
...
def binding():
    ...
    value.update(schema='microduck.contact-patch-freeze/v32',
    ...
    for name in ['scripts/probe_contact_patch_v32.py', 'experiments/walking/CONTACT-PATCH-v32.md', 'microduck/contact_patch_env_v32.py']:
...
def run(output, result):
    ...
    from microduck.contact_patch_env_v32 import ContactPatchWalkingEnv as PublicSurfaceWalkingEnv
    ...
    for index,(directory,session_id) in enumerate(CASES):
        ...
        item = dict(session_id=session_id,input_receipt=directory,steps=len(rows),physics_steps=len(history),
            ...
            native_plane_reproduced=True,genesis_variant='gjk-with-contact-patch',genesis_original_max_qpos_delta=observer_delta,comparisons={})
...
def main():
    ...
    result = dict(schema='microduck.contact-patch/v32',status='starting',cases=[],physical_calibration=False)
