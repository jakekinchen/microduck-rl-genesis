"""V55 selected recipe FINAL with V30 controller on exposed flat gates."""
import argparse
import gzip
import json
...
FREEZE=ROOT/"experiments/walking/yaw-flat-freeze-v55.json"
...
def main():
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "V55 final selected recipe in both legacy slots, V16 ramp and fixed V30 heading; exposed regression",
                       "source_sha256": {name: digest(ROOT/name) for name in SOURCES}}, f, indent=2)
            f.write("\n")
        return
    from experiments.walking.yaw_v55 import run_name
    ...
    if (record["status"] != "completed" or record["variant"] != "native-yaw-walking-v55"
        ...
        raise ValueError("exact bounded V55 component FINAL required")
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V55 {a.recipe} {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "native-yaw-v55-flat-regression",
              ...
              "controller": "Fixed V16 command slew and V30 float32 correction-headroom persistent-reference filtered IMU feedback; V55 selected recipe; shared recipe uses identical policies in both slots; exact-zero ramped-command routing",
