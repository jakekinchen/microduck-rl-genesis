...
from experiments.walking.snapshots_v43 import save_state, load_state

FREEZE = ROOT / "experiments/walking/handoff-coverage-freeze-v43-r3.json"
...
def binding():
    ...
    for name in ["scripts/probe_handoff_coverage_v43_r3.py", "scripts/probe_handoff_coverage_v43_r2.py", "scripts/probe_handoff_coverage_v43.py", "scripts/verify_walking_sequence_v42.py",
                 "experiments/walking/HANDOFF-COVERAGE-v43.md", "experiments/walking/snapshots_v43.py"]:
...
def main():
    ...
    if shutil.disk_usage(ROOT).free < 2 * 1024**3:
        raise ValueError("at least 2 GiB headroom required for bounded compressed diagnosis")
    ...
    def retain(world, state, identity, meta):
        dest = output / "states" / (identity + ".chunks.json")
        dest.parent.mkdir(exist_ok=True)
        save_state(state, dest, output / "snapshot-blobs")
        expected_sha = digest(dest)
        # Only self-created, hash-bound snapshots are deserialized.
        if digest(dest) != expected_sha:
            raise ValueError("snapshot changed")
        decoded = load_state(dest, output / "snapshot-blobs")
