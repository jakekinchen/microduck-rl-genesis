"""V41 native standing FINAL with V30 controller on exposed flat gates."""
import argparse
import gzip
import json
...
FREEZE=ROOT/"experiments/walking/native-standing-flat-freeze-v41.json"
...
SOURCES = list(dict.fromkeys(SOURCES + ["scripts/evaluate_native_standing_flat_v41.py", "scripts/train_native_standing_v41.py", "microduck/native_standing_env_v41.py", "experiments/walking/NATIVE-STANDING-v41.md", "experiments/walking/native_candidate_v41.py"]))
...
def main():
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "V21 FINAL walker, native V41 FINAL stander, V16 ramp and fixed V30 heading; exposed regression",
    ...
    stand, stand_parity, stand_export = export_walking("standing-native-20260906-v41", a.output/"standing")
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V41 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "native-standing-v41-flat-regression",
              ...
              "controller": "Fixed V16 command slew and V30 float32 correction-headroom persistent-reference filtered IMU feedback; V21 FINAL walking / native V41 FINAL standing; exact-zero ramped-command routing",
