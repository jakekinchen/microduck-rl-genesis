"""Bounded warm-start robust PPO, preserving all prior source/weight receipts."""
import argparse
import copy
...
def main():
    ...
    p.add_argument("--seed", type=int, default=26090501)
    args = p.parse_args()
    ...
    from microduck.laser_robust_env import MicroduckLaserRobustEnv, ROBUST_CONFIG
    checkpoint = ROOT/"receipts/laser-follow/20260904-v2-steering/source-checkpoint.pt"
    expected = "d5c6592881d061cd47c0c66b220cdebc9c9f43fe2a3d9f9fcff4d58703e827f6"
    ...
    sources = ["scripts/train_laser_robust.py", "microduck/laser_robust_env.py", "scripts/train_laser.py",
    ...
    record = {"schema": "microduck.laser-training/v1", "variant": "robust-v1", "proof_class": "first_party_development",
              "held_out": False, "target_source": "simulated-ground-truth", "args": vars(args),
              "train_cfg": cfg, "robustness": ROBUST_CONFIG, "new_transitions": args.num_envs*24*args.iterations,
              ...
              "warm_start": {"path": str(checkpoint.relative_to(ROOT)), "sha256": expected, "prior_transitions": 14745600},
              "status": "starting"}
    started = time.monotonic()
    try:
        (output/"run.json").write_text(json.dumps(record, indent=2)+"\n")
        gs.init(backend=gs.metal, logging_level="warning", seed=args.seed)
        env = MicroduckLaserRobustEnv(args.num_envs)
