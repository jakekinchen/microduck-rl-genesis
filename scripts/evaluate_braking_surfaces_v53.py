...
FREEZE=ROOT/"experiments/walking/braking-surfaces-freeze-v53.json"
...
from scripts.train_braking_v53 import EXTRA as BRAKE_SOURCES
...
def main():
    ...
    from microduck.braking_v53 import install,retain,RUN as BRAKE_RUN
    ...
    try:
                            ...
                            try:
                                for i in range(round(window["duration_s"]*50)):
                                    ...
                                    row.update(case_id=name,session_id=session["id"],session_time_s=row["time_s"],time_s=(i+1)*.02,
                                        actor_observation=world.last_observation[0].tolist(),braking_active=world.brake_v53.active,braking_age=world.brake_v53.age,
                                    ...
                                    expected += world.brake_v53.delta(world.last_observation)[0]
        ...
        result={"schema":"microduck.terrain-endurance-result/v1","bank":a.bank,"controller_variant":"V53-transient-braking-on-V50-V15","candidate":a.candidate,"proof_class":"visible_development",
