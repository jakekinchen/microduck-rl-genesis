"""Two separately invoked, bounded V56 standing-posture PPO arms; no automatic run extension."""
import argparse
import copy
...
from experiments.walking.posture_v56 import (
...
EXTRA = OLD_EXTRA + [
 'microduck/native_posture_env_v56.py', 'experiments/walking/posture_v56.py',
 'experiments/walking/POSTURE-REFINEMENT-v56.md', 'experiments/walking/posture-development-v56.json',
 'scripts/train_posture_v56.py', 'scripts/probe_posture_v56.py', 'scripts/probe_posture_feasibility_v56.py', 'experiments/walking/posture-feasibility-v56.json', 'scripts/verify_posture_v56.py',
 'scripts/evaluate_posture_flat_v56.py', 'scripts/evaluate_posture_endurance_v56.py',
 'scripts/evaluate_posture_surfaces_v56.py', 'tests/test_posture_v56.py']
from experiments.walking.posture_v56 import INITIAL_RUN, INITIAL_SHA, WEIGHTS


def binding():
    from scripts.train_yaw_v55 import require_freeze as old_freeze
    base=old_freeze()
    ...
    return dict(schema='microduck.posture-freeze/v56',source_sha256=sources,
        warm_start=dict(run=INITIAL_RUN,checkpoint='model_1499.pt',sha256=INITIAL_SHA),
        seed=SEED,recipes=list(RECIPES),face_weights=WEIGHTS,
        full_transitions_per_arm=2_592_000,smoke_transitions_per_arm=2880)


def require_freeze():
    frozen=json.loads(FREEZE.read_text())
    if binding()!=frozen: raise ValueError('V56 freeze drift')
    return frozen
...
def main():
    ...
    probe=ROOT/'receipts/walking/20260916-v56-conformance'
    from scripts.analyze_components_v45 import verify_manifest
    feasibility=ROOT/'receipts/walking/20260916-v56-posture-feasibility'
    verify_manifest(feasibility)
    feasible=json.loads((feasibility/'feasibility.json').read_text())
    if (not feasible['complete'] or not feasible['policies_with_all_six_feasible']
            or feasible['protocol_sha256']!=digest(ROOT/'experiments/walking/posture-feasibility-v56.json')):
        raise ValueError('complete, passing standing feasibility required before training')
    ...
    if not proof['complete'] or not proof['passed'] or proof['freeze_sha256']!=digest(FREEZE):
        raise ValueError('complete V56 conformance required')
    ...
    record=dict(schema='microduck.walking-training/v1',variant='native-posture-v56',
        ...
        initialization_sha256=INITIAL_SHA,face_weight=WEIGHTS[args.recipe],
    ...
    try:
        ...
        from microduck.native_posture_env_v56 import NativeSequenceEnv
        ...
        env=NativeSequenceEnv(num_envs,SEED,out,face_weight=WEIGHTS[args.recipe]);record['env_cfg']=env.cfg
        ...
        def stop(signum,frame): raise KeyboardInterrupt(f'owned V56 job interrupted: {signum}')
    ...
    for role in PARENTS:
        ...
        component.update(variant='native-posture-'+role+'-v56',joint_checkpoint_sha256=record['checkpoint_sha256'])
