...
def main():
    ...
    p.add_argument("--feedback-policy",type=Path,help="Separate labeled diagnostic: infer fresh actions instead of fixed replay")
    a=p.parse_args()
    ...
    env.reset();obs=env.place([0,0,.125],yaw=0)
    if a.feedback_policy:
        import onnxruntime as ort
        from microduck.laser_dynamics import program_target
        from microduck.laser_task import world_to_body_xy
        from microduck.laser_steering import approach_command
        session=ort.InferenceSession(str(a.feedback_policy),providers=["CPUExecutionProvider"])
    ...
    with (a.output/"trajectory.jsonl").open("w") as stream:
        for i,action in enumerate(actions[:round(a.seconds*50)]):
            if a.feedback_policy:
                target,visible,_,_=program_target("circle",i*.02)
                command=approach_command(world_to_body_xy(target,env.base_pos[0,:2].cpu().numpy(),env.base_quat[0].cpu().numpy()),visible,3.)
                env.laser_xy[0]=torch.from_numpy(target).to(env.device)
                env.laser_visible[:]=visible
                vector=obs["policy"].cpu().numpy().copy();vector[:,48:51]=command
                action=session.run(None,{"obs":vector})[0][0]
            tensor=torch.from_numpy(action[None].copy()).to(env.device)
            obs,_,done,_=env.step(tensor)
    ...
    result["first_root_below_7cm_s"]=next((r["time_s"] for r in rows if r["robot_xyz_m"][2]<.07),None)
    if a.feedback_policy:
        result.update(schema="microduck.genesis-gait-feedback/v1",policy_sha256=digest(a.feedback_policy),
                      original_tensor_bytes_equal_on_replayed_prefix=None,
                      boundary="Feedback diagnostic in the training engine. Actions differ from the MuJoCo path; not a physics-isolation replay or physical acceptance.")
