"""Bounded source-preserving local walking-v2 training."""
import argparse
import copy
...
def main():
    ...
    p.add_argument("--seed",type=int,default=26090512)
    a=p.parse_args()
    ...
    checkpoint=ROOT/"receipts/walking/20260905-v1-stopped/model_250.pt"
    expected="c1ab3f1a8cb028c8c41426dd7c6eb4bffdd8dc5da837893f01ff912ba30efe8d"
    record={"schema":"microduck.walking-training/v1","variant":"walking-v2",
    ...
    try:
        ...
        prior=json.loads((ROOT/"receipts/walking/20260905-v1-stopped/original-run.json").read_text())
        sources=list(prior["source_sha256"])+["scripts/train_walking_controlled.py",
            "microduck/walking_controlled_env.py","experiments/walking/CONTROLLED-v2.md"]
        ...
        env=MicroduckControlledWalkingEnv(a.num_envs);record["env_cfg"]=env.cfg
        ...
        def stop(signum,frame):raise KeyboardInterrupt(f"owned walking-v2 run interrupted: {signum}")
