...
FREEZE = ROOT/'experiments/walking/exact-patch-freeze-v35.json'
...
def binding():
    ...
    value.update(schema='microduck.exact-patch-freeze/v35',
    ...
    for name in ['scripts/probe_exact_patch_v35.py', 'experiments/walking/EXACT-PATCH-v35.md', 'microduck/exact_patch_env_v35.py', 'experiments/walking/collider_geometry_v34.py']:
...
def run(output, result):
    ...
    from microduck.exact_patch_env_v35 import ExactPatchWalkingEnv as PublicSurfaceWalkingEnv
    ...
    for index,(directory,session_id) in enumerate(CASES):
        ...
        item = dict(session_id=session_id,input_receipt=directory,steps=len(rows),physics_steps=len(history),
            ...
            native_plane_reproduced=True,genesis_variant='exact-hull-gjk-patch',genesis_original_max_qpos_delta=observer_delta,comparisons={})
...
def main():
    ...
    result = dict(schema='microduck.exact-patch/v35',status='starting',cases=[],physical_calibration=False)
