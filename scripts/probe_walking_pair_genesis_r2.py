...
FREEZE = ROOT / "experiments/walking/paired-genesis-freeze-v20-r2.json"
...
def binding():
    ...
    for name in ("scripts/probe_walking_pair_genesis_r2.py",
    ...
    for timing_id, case_id in CASES:
        ...
        with (output / (name + ".jsonl")).open("w") as stream:
            for i in range(900):
                ...
                env.set_twist(*(float(value) for value in command))
