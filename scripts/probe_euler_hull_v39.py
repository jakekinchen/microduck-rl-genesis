...
FREEZE = ROOT/'experiments/walking/euler-hull-freeze-v39.json'
...
def binding():
    ...
    value.update(schema='microduck.euler-hull-freeze/v39',
    ...
    for name in ['scripts/probe_euler_hull_v39.py', 'experiments/walking/EULER-HULL-v39.md', 'microduck/euler_hull_env_v39.py', 'experiments/walking/collider_geometry_v34.py']:
...
def run(output, result):
    ...
    from microduck.euler_hull_env_v39 import EulerHullWalkingEnv as PublicSurfaceWalkingEnv
    ...
    for index,(directory,session_id) in enumerate(CASES):
        ...
        item = dict(session_id=session_id,input_receipt=directory,steps=len(rows),physics_steps=len(history),
            ...
            native_plane_reproduced=True,genesis_variant='euler-exact-hull-mpr',genesis_original_max_qpos_delta=observer_delta,comparisons={})
...
def main():
    ...
    result = dict(schema='microduck.euler-hull/v39',status='starting',cases=[],physical_calibration=False)
