"""Read complete V56 evidence and decide prerequisites without rerunning physics."""
import gzip
import hashlib
import json
...
from experiments.walking.posture_v56 import (FREEZE,RECIPES,REPLAY,candidate,digest,
...
def verify_evaluation(folder,recipe,expected_count,session_bank):
    ...
    if recipe in ('control','face2','parent'):
...
def main():
    ...
    out=new_output('receipts/walking/20260916-v56-verification')
    init=ROOT/'logs'/INITIAL_RUN;verify_manifest(init)
    init_record=json.loads((init/'run.json').read_text())
    assert init_record['status']=='completed' and init_record['checkpoint_sha256']==INITIAL_SHA
    parent=torch.load(init/'model_1499.pt',map_location='cpu',weights_only=True)
    parent_fresh,parent_parity=verify_evaluation(ROOT/'receipts/walking/20260916-v56-parent-fresh','parent',2,True)
    ...
    for recipe in RECIPES:
        ...
        for group in ['actor_state_dict','critic_state_dict']:
            for key,value in parent[group].items():
                assert value.numpy().tobytes()==initial[group][key].numpy().tobytes()
        ...
        with gzip.open(folder/'all-reward-components-float32.bin.gz','rb') as stream:
            rewards=np.frombuffer(stream.read(),dtype='<f4').reshape(-1,record['args']['num_envs'],8)
        ...
        np.testing.assert_allclose(rewards[:,:,4],rewards[:,:,3]-.02*(rewards[:,:,2]+rewards[:,:,7]),rtol=3e-5,atol=3e-5)
        expected_face=np.where(rewards[:,:,5]!=0,WEIGHTS[recipe]*(np.maximum(abs(rewards[:,:,6])-25,0)/5)**2,0)
        np.testing.assert_allclose(rewards[:,:,7],expected_face,rtol=3e-5,atol=3e-5)
        ...
        assert scalar_count>=2250
        banks={};parity={}
        for bank,count,is_session in [('flat',21,False),('repeated',42,False),('endurance',4,True),('surfaces',14,True),('fresh',2,True)]:
            path=ROOT/'receipts/walking'/f'20260916-v56-{recipe}-{bank}'
        ...
        behavior=(banks['flat']['passed_cases']==21 and banks['repeated']['passed_cases']==42
            and banks['endurance']['passed_sessions']==4 and banks['fresh']['passed_sessions']==2 and old_passes<=new_passes)
    ...
    for bank in ['flat','endurance','surfaces']:
        verify_sources(json.loads((ROOT/f'experiments/walking/yaw-{bank}-freeze-v56.json').read_text()))
    require_freeze()
    result=dict(complete=True,evidence_verified=True,freeze_sha256=digest(FREEZE),
        initializer=init_record,arms=reports,identical_warm_actor_and_critic=True,parent_fresh_parity=parent_parity,parent_fresh_passed_sessions=parent_fresh['passed_sessions'],
