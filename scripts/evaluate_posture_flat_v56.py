"""V56 selected recipe FINAL with V30 controller on exposed flat gates."""
import argparse
import gzip
import json
...
FREEZE=ROOT/"experiments/walking/posture-flat-freeze-v56.json"
...
def main():
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "V56 final selected recipe in both legacy slots, V16 ramp and fixed V30 heading; exposed regression",
                       "source_sha256": {name: digest(ROOT/name) for name in SOURCES}}, f, indent=2)
            f.write("\n")
        return
    from experiments.walking.posture_v56 import run_name
    ...
    if (record["status"] != "completed" or record["variant"] != "native-posture-walking-v56"
            or record["checkpoint"] != "model_2249.pt" or record["new_transitions"] != 2_592_000
            or record["args"]["num_envs"] != 48 or record["args"]["iterations"] != 2250):
        raise ValueError("exact bounded V56 component FINAL required")
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V56 {a.recipe} {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "native-posture-v56-flat-regression",
              ...
              "controller": "Fixed V16 command slew and V30 float32 correction-headroom persistent-reference filtered IMU feedback; V56 selected recipe; shared recipe uses identical policies in both slots; exact-zero ramped-command routing",
