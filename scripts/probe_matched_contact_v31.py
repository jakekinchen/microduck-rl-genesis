...
FREEZE = ROOT/'experiments/walking/matched-contact-freeze-v31.json'
PRIOR = ROOT/'receipts/walking/20260906-v25-action-replay'


def binding():
    value = prior_binding()
    value.update(schema='microduck.matched-contact-freeze/v31',
    ...
    for name in ['scripts/probe_matched_contact_v31.py', 'experiments/walking/MATCHED-CONTACT-v31.md']:
...
def run(output, result):
    ...
    for index,(directory,session_id) in enumerate(CASES):
        ...
        np.testing.assert_allclose([r['qpos'] for r in history[3::4]],[r['qpos'] for r in old],rtol=0,atol=1e-7)
        ...
        item = dict(session_id=session_id,input_receipt=directory,steps=len(rows),physics_steps=len(history),
            ...
            native_plane_reproduced=True,genesis_observer_reproduced=True,comparisons={})
...
def main():
    ...
    result = dict(schema='microduck.matched-contact/v31',status='starting',cases=[],physical_calibration=False)
