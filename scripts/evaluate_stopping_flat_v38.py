"""V38 translational STOP deceleration on the exposed flat bank."""
import argparse
import gzip
import json
...
FREEZE=ROOT/"experiments/walking/stopping-flat-freeze-v38.json"
...
SOURCES = list(dict.fromkeys(SOURCES + ["scripts/evaluate_stopping_flat_v38.py", "microduck/stopping_ramp_v38.py", "experiments/walking/STOPPING-RAMP-v38.md", "tests/test_stopping_ramp_v38.py"]))
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "V21 FINAL walker, V15 FINAL stander, V38 stopping ramp and fixed V30 heading; exposed regression",
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V38 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "stopping-ramp-v38-flat-regression",
              ...
              "controller": "V38 translational STOP deceleration and V30 float32 correction-headroom persistent-reference filtered IMU feedback; V21 FINAL walking / V15 FINAL standing; exact-zero ramped-command routing",
