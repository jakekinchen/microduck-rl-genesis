...
def main():
    ...
    for suffix,baseline,filename in pairs:
        candidate=ROOT/f'receipts/walking/20260907-v44-sequence-{suffix}'
    ...
    trace_data={}
    for label,folder in [('V30 retained','20260906-v30-flat-regression'),('V44 candidate','20260907-v44-sequence-flat')]:
        ...
        for actor in ['walking','standing']:
            ...
            role[actor]=dict(controls=len(selected),fraction_below_5_percent=float((margin<.05).mean()))
    ...
    for label,color in [('V30 retained','#1d796a'),('V44 candidate','#bc4932')]:
    ...
    summary.update(candidate_rejected=True,fresh_sequence_bank_opened=False,physical_calibration=False,
        diagnostic_case=case,phase_occupancy={k:v['phase_occupancy'] for k,v in trace_data.items()},
        interpretation='The first case has excessive right-knee near-limit occupancy during walking. This phase association does not by itself isolate the learned walker from incoming state supplied by the stander.')
