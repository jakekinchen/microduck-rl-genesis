"""V56 selected recipe FINAL on unchanged surfaces gates."""
import argparse
import gzip
...
FREEZE=ROOT/"experiments/walking/posture-surfaces-freeze-v56.json"
...
from scripts.train_posture_v56 import EXTRA
...
def main():
    ...
    p.add_argument("--recipe",choices=("control","face2"))
    a=p.parse_args()
    ...
    from experiments.walking.posture_v56 import candidate
    ...
    try:
                            ...
                            try:
                                for i in range(round(window["duration_s"]*50)):
                                    ...
                                    if writer and i%2==0:
                                        ...
                                        draw.text((8,8),f'V56 {a.recipe} | {name} | {row["session_time_s"]:.2f}s | {role}',fill="white")
        ...
        result={"schema":"microduck.terrain-endurance-result/v1","bank":a.bank,"controller_variant":"V30-with-native-posture-v56","candidate":a.recipe,"proof_class":"visible_development",
                "recipe":a.recipe,"shared_policy_in_both_slots":True,
