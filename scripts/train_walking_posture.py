"""Bounded source-preserving local walking-v3 training."""
import argparse
import copy
...
def main():
    ...
    p.add_argument("--seed",type=int,default=26090513)
    a=p.parse_args()
    ...
    checkpoint=ROOT/"receipts/walking/20260905-v2-stopped/model_250.pt"
    expected="1742d3b4dccc12a6437158ade211984a4282b7339f9a32ef1216acdfb69ab4c6"
    record={"schema":"microduck.walking-training/v1","variant":"walking-v3",
    ...
    try:
        ...
        prior=json.loads((ROOT/"receipts/walking/20260905-v2-stopped/original-run.json").read_text())
        sources=list(prior["source_sha256"])+["scripts/train_walking_posture.py",
            "microduck/walking_posture_env.py","experiments/walking/POSTURE-v3.md"]
        ...
        env=MicroduckPostureWalkingEnv(a.num_envs);record["env_cfg"]=env.cfg
        ...
        def stop(signum,frame):raise KeyboardInterrupt(f"owned walking-v3 run interrupted: {signum}")
