"""Bounded persistent-delay complete-contact v13, retained v9 FINAL initializer only."""
import argparse
import copy
...
def initializer(root):
    ...
    controller = root/"receipts/walking/20260905-v12-current-sensor"
    verify_input_manifest(controller)
    controlled = json.loads((controller/"evaluation.json").read_text())
    if (controlled["acceptance_variant"] != "imu-heading-command-servo-v12"
            or controlled["total_cases"] != 21 or controlled["passed_cases"] != 17
            or not all(c["heading"]["passed"] and c["self_contact"]["passed"] for c in controlled["case_reports"])
            or controlled["policy_sha256"] != evaluation["policy_sha256"]):
        raise ValueError("retained 17/21 v12 timing-negative baseline required")
...
def main():
    ...
    p.add_argument("--seed", type=int, default=26090523)
    a = p.parse_args()
    if (not a.run_id.replace("-", "").isalnum() or not 1 <= a.num_envs <= 1024
            or not 1 <= a.iterations <= 750 or a.seed != 26090523):
        p.error("simple new run ID, at most 1024x750, frozen seed 26090523")
    checkpoint, prior, baseline_sha = initializer(ROOT)
    from scripts.evaluate_walking_persistent_controller import FREEZE, SOURCES
    ...
    record = {"schema": "microduck.walking-training/v1", "variant": "walking-v13",
    ...
    try:
        ...
        sources = list(dict.fromkeys(list(prior["source_sha256"])+SOURCES+[
            "scripts/train_walking_persistent_contact.py", "microduck/walking_collision_env.py",
            "microduck/walking_persistent_contact_env.py", "microduck/walking_persistent_timing_env.py",
            "tests/test_walking_persistent_contact.py", "tests/test_walking_persistent_timing.py",
            "experiments/walking/PERSISTENT-CONTACT-v13.md",
            "experiments/walking/evaluator-freeze-persistent-controller-v13.json"]))
        ...
        env = MicroduckPersistentCompleteWalkingEnv(a.num_envs,
        ...
        def stop(signum, frame):
            raise KeyboardInterrupt(f"owned v13 run interrupted: {signum}")
