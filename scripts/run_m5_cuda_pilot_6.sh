...
check_disk_preflight() {
  python3 - "$WORKSPACE_ROOT" "$MINIMUM_FREE_DISK_GB" "$RECEIPT_ROOT/disk-preflight.json" <<'PY'
import json
import shutil
import sys
from pathlib import Path
...
result = {
    ...
    "passed": free_bytes >= minimum_gb * 1024**3,
...
test -s "$INPUT_ROOT/run_m5_cuda_pilot_6.sh"
cp "$INPUT_ROOT/run_m5_cuda_pilot_6.sh" "$RECEIPT_ROOT/run_m5_cuda_pilot_6.sh"
sha256sum "$RECEIPT_ROOT/run_m5_cuda_pilot_6.sh" > "$RECEIPT_ROOT/harness-sha256.txt"

run_logged host-disk-preflight check_disk_preflight
