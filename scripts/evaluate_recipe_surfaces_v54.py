"""V54 selected recipe FINAL on unchanged surfaces gates."""
import argparse
import gzip
...
FREEZE=ROOT/"experiments/walking/recipe-surfaces-freeze-v54.json"
...
from scripts.train_recipe_v54 import EXTRA
...
def main():
    ...
    p.add_argument("--recipe",choices=("routed","shared"))
    a=p.parse_args()
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            ...
            f.write("\n")
        return
    if a.recipe is None: p.error("recipe required")
    ...
    from experiments.walking.recipe_v54 import candidate
    WALK_RUN,STAND_RUN,(walk_sha,stand_sha)=candidate(a.recipe)
    ...
    try:
                            ...
                            try:
                                for i in range(round(window["duration_s"]*50)):
                                    ...
                                    if writer and i%2==0:
                                        ...
                                        draw.text((8,8),f'V54 {a.recipe} | {name} | {row["session_time_s"]:.2f}s | {role}',fill="white")
        ...
        result={"schema":"microduck.terrain-endurance-result/v1","bank":a.bank,"controller_variant":"V30-with-native-recipe-v54","candidate":a.recipe,"proof_class":"visible_development",
                "recipe":a.recipe,"shared_policy_in_both_slots":a.recipe=="shared",
