...
FREEZE = ROOT / "experiments/walking/handoff-coverage-freeze-v43.json"
...
def binding():
    ...
    for name in ["scripts/probe_handoff_coverage_v43.py", "scripts/verify_walking_sequence_v42.py",
            ...
            with opener(path, "rt") as stream:
                for session_id, group in itertools.groupby((json.loads(line) for line in stream), lambda r: r["session_id"]):
                    ...
                    for saved in group:
                        ...
                        current = world.step_command(saved["command"], action_override=action)
