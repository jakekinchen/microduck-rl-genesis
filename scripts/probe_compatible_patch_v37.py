...
FREEZE = ROOT/'experiments/walking/compatible-patch-freeze-v37.json'
...
def binding():
    ...
    value.update(schema='microduck.compatible-patch-freeze/v37',
    ...
    for name in ['scripts/probe_compatible_patch_v37.py', 'experiments/walking/COMPATIBLE-PATCH-v37.md', 'microduck/compatible_patch_env_v37.py', 'experiments/walking/collider_geometry_v34.py']:
...
def run(output, result):
    ...
    from microduck.compatible_patch_env_v37 import CompatiblePatchWalkingEnv as PublicSurfaceWalkingEnv
    ...
    for index,(directory,session_id) in enumerate(CASES):
        ...
        item = dict(session_id=session_id,input_receipt=directory,steps=len(rows),physics_steps=len(history),
            ...
            native_plane_reproduced=True,genesis_variant='compatible-exact-hull-gjk-patch',genesis_original_max_qpos_delta=observer_delta,comparisons={})
...
def main():
    ...
    result = dict(schema='microduck.compatible-patch/v37',status='starting',cases=[],physical_calibration=False)
