...
The dated briefs below preserve the original decisions. Their "next" wording
is historical and does not override the current owner priority.

---

# V55 complete; standing-posture objective ablation is next

September 12: both fixed main runs and all 11 evaluation banks are complete.
Neither final passes admission. Both retain 63 flat cases and the original
five surface passes. Yaw6 survives all four downhill sessions but every
window fails yaw/final face pitch, and one full composition fails whole-session
heading by .0982 degrees. Both arms miss the same two long-training conditions.
All action/physics-load/source evidence verifies; no compute remains running.

Read `experiments/walking/YAW-RESULTS-v55.md` and its recorded videos/plots.
Next freeze one standing-only face-posture objective comparison from V54 shared,
with all existing yaw, physics, ABI, retention and curriculum settings fixed.
The current individual-joint cost is small in the failed final standing states;
this motivates an objective-alignment test, not a promised fix. Keep V55 yaw6
as an unaccepted survival comparator. V30 remains retained.

The workspace packet binds V30's local flat reference. V55 external receipts
are verified directly and remain unavailable to the workspace's symlink-safe
viewer. No physical or calibrated-carpet acceptance follows this result.

# V54 complete; shared-policy yaw refinement is next

The September 9 comparison and complete evidence audit are finished. Read
`experiments/walking/RECIPE-RESULTS-v54.md`. Shared retains 63/63 flat cases,
both complete 180-second compositions and the original five surface passes.
Both full downhill sessions survive repeated stops; yaw and one final face-pitch
gate still fail. Both recipes miss six long-training cells. Neither advances.

Next freeze a matched control and one yaw-objective refinement from the shared
checkpoint. Preserve the new stopping behavior and every old regression gate.
The recorded yaw error is mostly oscillatory, with little signed bias. Keep
posture and all-cell long coverage as separate open gates. Replication and wider
terrain remain conditional. The offline residual proposal below is historical.

The selected workspace packet binds the retained V30 flat reference. V54 bulk
evidence is on the UUID-verified external drive and is checked by its dedicated
verifier. The workspace viewer refuses symlinks; `duck prepare` reports this
inspection gap without opening external records. V30 remains retained and no
physical or calibrated-carpet claim is made.

# V53 retention complete; joint fitting feasibility is next
...
The active packet binds V53's passing nominal forward-0.08 flat reference.
