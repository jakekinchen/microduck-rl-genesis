"""Face-first gait-v3: learn stepping, not endpoint-only translation.
...
GAIT_CONFIG = {
    "version": "laser-gait-v3", "forward_axis": "body -X; HOME face direction",
...
class MicroduckLaserGaitEnv(MicroduckLaserRobustEnv):
    def __init__(self, num_envs, **kwargs):
        ...
        self.cfg.update(task="Laser-Face-First-Gait-v3", gait=GAIT_CONFIG,
                        reward_version="gait-v3", reward_weights=dict(self.reward_weights))
    ...
    def _resample_twist(self, env_ids):
        ...
        # A moving target is introduced only after initial negative-vx stepping.
        progress = min(self.common_step_counter/(300*24), 1.)
        span = .25 + (math.pi-.25)*progress
        angle = yaw[env_ids]+math.pi+torch.empty(len(env_ids), device=self.device).uniform_(-span, span)
    ...
    def _rew_track_lin(self):
        ...
        return score*torch.where(self._command_active(), self._gait_credit(), torch.ones_like(score))
