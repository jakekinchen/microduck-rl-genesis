"""Composite target AND gait development evaluation for the explicit v3 lane.
...
def main():
    ...
    spec_path=ROOT/"experiments/laser/gait-correction-v3.json"
    ...
    with (a.output/"trajectory.jsonl").open("w") as stream:
        for i,case in enumerate(spec["visible_cases"]):
            world=FaceFirstLaserWorld(policy,a.bam_repo,domain_draw(76041+i,False),render=a.video)
            ...
            try:
                ...
                world.close()
    result={"schema":"microduck.laser-gait-evaluation/v3","proof_class":"visible_development",
    ...
    for source in ("scripts/evaluate_laser_gait.py","experiments/laser/gait.py","experiments/laser/world.py","experiments/laser/face_world.py","microduck/laser_face_command.py"):
