...
def main():
    ...
    p.add_argument("--approach-gain", type=float, choices=(1.5, 3.0), default=1.5,
                   help="versioned command adapter; never changes policy actions or suite thresholds")
    args = p.parse_args()
        ...
        for case in suite["cases"]:
            ...
            try:
                for step in range(round(case["seconds"]*50)):
                    ...
                    command = approach_command(relative, visible, args.approach_gain)
    ...
    result = {"schema": "microduck.laser-evaluation/v1", "proof_class": "first_party_development",
              ...
              "steering": {"id": "laser-steering-v1" if args.approach_gain == 1.5 else "laser-steering-approach-v2",
                           "distance_gain_s_inv": args.approach_gain,
                           "source_sha256": digest(ROOT/"microduck/laser_steering.py"),
                           "policy_actions_postprocessed": False},
    ...
    for source in ("scripts/evaluate_laser.py", "microduck/laser_steering.py"):
