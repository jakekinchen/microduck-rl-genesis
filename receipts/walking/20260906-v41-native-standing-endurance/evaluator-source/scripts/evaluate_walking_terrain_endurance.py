"""Preregistered V23 native terrain/endurance with exact V21/V15 components."""
import argparse
...
SUITE=ROOT/"experiments/walking/terrain-endurance-v23.json"
FREEZE=ROOT/"experiments/walking/terrain-endurance-freeze-v23.json"
SOURCES=list(dict.fromkeys(PRIOR+[
    "scripts/evaluate_walking_terrain_endurance.py", "experiments/walking/terrain_v23.py",
    "experiments/walking/endurance_v23.py", "tests/test_walking_terrain_v23.py",
    "tests/test_walking_endurance_v23.py", "experiments/walking/terrain-endurance-v23.json",
    "experiments/walking/TERRAIN-ENDURANCE-v23.md"]))
...
def main():
    ...
    p.add_argument("--bank",choices=("terrain","endurance"))
                ...
                try:
                    with record_self_loads(world) as loads:
                            ...
                            try:
                                for i in range(round(window["duration_s"]*50)):
                                    ...
                                    if writer and i%2==0:
                                        ...
                                        draw.text((8,8),f'V23 | {name} | {row["session_time_s"]:.2f}s | {role}',fill="white")
                        ...
                        session_reports.append({"session_id":session["id"],"bucket":session["bucket"],
                            "passed":all(r["passed"] for r in session_cases) and continuity["passed"] and exposure,
                            "failures":(["window_failure"] if not all(r["passed"] for r in session_cases) else [])+
