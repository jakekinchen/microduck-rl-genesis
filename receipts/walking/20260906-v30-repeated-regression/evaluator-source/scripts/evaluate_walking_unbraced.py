"""V18 load-aware walker, V15 stander, V16 slew; unchanged user-command gates."""
import argparse
import json
...
SOURCES = list(dict.fromkeys(PRIOR_SOURCES + ["scripts/evaluate_walking_unbraced.py",
...
FREEZE = ROOT/"experiments/walking/evaluator-freeze-unbraced-v18-r2.json"
...
def main():
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "V18 FINAL walker and V15 FINAL stander with V16 ramp; V18 training not yet started",
    ...
    if a.output is None or not a.run_id or not a.run_id.replace("-", "").isalnum():
        p.error("completed V18 run ID and new output required")
    ...
    if (record["status"] != "completed" or record["variant"] != "walking-v18"
            or record["checkpoint"] != "model_249.pt" or record["new_transitions"] != 6_144_000
            or record["args"]["seed"] != 26090618):
        raise ValueError("exact bounded V18 FINAL required")
    ...
    from experiments.walking.command_ramp import RampedStandingSwitchWorld as StandingSwitchWalkingWorld
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V18 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "unbraced-walking-ramped-v18",
              ...
              "controller": "Fixed V16 command slew and V12 IMU feedback; V18 FINAL walking / V15 FINAL standing; exact-zero ramped-command routing",
