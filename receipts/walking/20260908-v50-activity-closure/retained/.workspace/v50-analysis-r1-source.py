...
def main():
    ...
    shift=[]
    for i in range(len(replay['observations'])):
        action=session.run(None,{session.get_inputs()[0].name:replay['observations'][i:i+1]})[0][0]
        assert np.isfinite(action).all()
        shift.append(abs(action-replay['actions'][i]))
