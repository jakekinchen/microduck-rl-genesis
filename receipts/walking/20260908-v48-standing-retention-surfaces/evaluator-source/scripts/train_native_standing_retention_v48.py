"""One bounded standing-only PPO correction with explicit flat/endurance retention."""
import argparse
import copy
...
FREEZE=ROOT/'experiments/walking/native-standing-retention-freeze-v48.json'
PARENTS={'walking':'walking-20260906-v21','standing':'standing-20260906-v15'}
EXTRA=['microduck/retained_stander_v48.py','microduck/native_sequence_env_v48.py','microduck/native_sequence_env_v46.py',
       'scripts/train_native_standing_retention_v48.py','scripts/probe_native_sequence_v48.py',
       'scripts/build_standing_replay_v48.py','scripts/verify_handoff_inspection_v47.py','experiments/walking/handoff_inspection_v47.py',
       'experiments/walking/HANDOFF-RESULTS-v47.md',
       'experiments/walking/STANDING-RETENTION-v48.md','experiments/walking/native_candidate_v48.py',
       'experiments/walking/standing-replay-v48/replay.npz','experiments/walking/standing-replay-v48/provenance.json',
       'scripts/evaluate_native_standing_retention_flat_v48.py','scripts/evaluate_native_standing_retention_endurance_v48.py',
       'scripts/evaluate_native_standing_retention_surfaces_v48.py','tests/test_retained_stander_v48.py']
...
def binding():
    ...
    return dict(schema='microduck.native-standing-retention-freeze/v48',source_sha256=sources,
        warm_start=warm,seed=26090848,full_transitions=576000,smoke_transitions=2880)


def main():
    p=argparse.ArgumentParser();p.add_argument('--freeze',action='store_true');p.add_argument('--run-id')
    p.add_argument('--num-envs',type=int,default=48);p.add_argument('--iterations',type=int,default=500);a=p.parse_args()
    frozen=binding()
    if a.freeze:
        with FREEZE.open('x') as f:json.dump(frozen,f,indent=2);f.write('\n')
        return
    if not a.run_id or not a.run_id.replace('-','').isalnum() or (a.num_envs,a.iterations) not in [(24,5),(48,500)]:p.error('24x5 smoke or 48x500 full')
    if frozen!=json.loads(FREEZE.read_text()):raise ValueError('freeze drift')
    preflight=ROOT/'receipts/walking/20260908-v48-conformance'
    ...
    if shutil.disk_usage(ROOT).free<3_000_000_000:raise ValueError('less than 3 GB free before bounded run')
    out=ROOT/'logs'/a.run_id;out.mkdir(parents=True,exist_ok=False)
    record=dict(schema='microduck.walking-training/v1',variant='native-standing-retention-v48',proof_class='first_party_development',
    ...
    try:
        ...
        from microduck.native_sequence_env_v48 import NativeSequenceEnv
        torch.set_num_threads(1);torch.manual_seed(26090848);np.random.seed(26090848)
        cfg=copy.deepcopy(TRAIN_CFG);cfg.update(seed=26090848,run_name=a.run_id,save_interval=500)
        cfg['algorithm'].update(learning_rate=3e-5,schedule='fixed',entropy_coef=0.,gamma=.999)
        cfg['obs_groups']['critic']=['policy']
        cfg['actor']['class_name']='microduck.retained_stander_v48:RetainedStanderActor'
        cfg['algorithm']['class_name']='microduck.retained_stander_v48:StandingRetentionPPO'
        cfg['actor']['distribution_cfg']['std_range']=[.005,.25]
        record['train_cfg']=copy.deepcopy(cfg)
        env=NativeSequenceEnv(a.num_envs,26090848,out);record['env_cfg']=env.cfg
        ...
        for role,run in PARENTS.items():
            ...
            if role=='standing':
                ...
                with torch.no_grad():actor.distribution.std_param.mul_(.5)
        data=np.load(ROOT/'experiments/walking/standing-replay-v48/replay.npz')
        ...
        immutable={role:{k:v.detach().cpu().numpy().tobytes() for k,v in getattr(runner.alg.actor,role).state_dict().items()} for role in ['walking','teacher']}
        def stop(signum,frame):raise KeyboardInterrupt(f'owned native standing retention training interrupted: {signum}')
    ...
    for role in PARENTS:
        ...
        component={**record,'variant':'native-standing-retention-'+role+'-v48','joint_checkpoint_sha256':record['checkpoint_sha256']}
