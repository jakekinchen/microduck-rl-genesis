"""V15 fixed walker / trained stander; unchanged gates and optional fresh repetitions."""
import argparse
import json
...
SOURCES = list(dict.fromkeys(PRIOR_SOURCES + ["scripts/evaluate_walking_standing_trained.py",
    "scripts/train_standing.py", "microduck/standing_env.py", "tests/test_standing_reward.py",
    "experiments/walking/STANDING-v15.md"]))
FREEZE = ROOT/"experiments/walking/evaluator-freeze-standing-trained-v15.json"
WALK_SHA = "f31d47a5343b1283a1bbd28c2f7efb78f95ddd6940554b9c755b73d337c2b7f8"
...
def main():
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "V13 FINAL walker and V15 FINAL stander, standing training not yet started",
    ...
    if a.output is None or not a.run_id or not a.run_id.replace("-", "").isalnum():
        p.error("completed V15 run ID and new output required")
    ...
    if (record["status"] != "completed" or record["variant"] != "standing-v15"
            or record["checkpoint"] != "model_249.pt" or record["new_transitions"] != 6_144_000
            or record["args"]["seed"] != 26090615):
        raise ValueError("exact bounded V15 FINAL required")
    ...
    if a.fresh:
        ...
        current = ROOT/"receipts/walking/20260906-v15-standing-pair"
        verify_input_manifest(current)
        current_result = json.loads((current/"evaluation.json").read_text())
        current_stand = json.loads((current/"standing/training.json").read_text())
        if (current_result["passed_cases"] != 21 or current_result["total_cases"] != 21
                or current_result["evaluator_freeze_sha256"] != digest(FREEZE)
                or current_stand["checkpoint_sha256"] != record["checkpoint_sha256"]):
            raise ValueError("same exact pair must pass every current gate before fresh development")
        ...
        if digest(suite_path) != json.loads(fresh_freeze.read_text())["suite_sha256"]:
            raise ValueError("fresh bank changed")
    import numpy as np
    import torch
    ...
    walk, walk_parity, walk_export = export_walking("walking-20260905-v13", a.output)
    stand, stand_parity, stand_export = export_walking(a.run_id, a.output/"standing")
    if json.loads((a.output/"training.json").read_text())["checkpoint_sha256"] != WALK_SHA:
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V15 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "command-stand-switch-v15",
              ...
              "controller": "Unchanged V12 IMU command servo; V13 FINAL walking / V15 FINAL standing, exact-zero command routing",
