...
- [ ] Target longest-delay downhill braking and the standing handoff with a
  newly frozen retention-constrained experiment. Inspect the earliest divergence
  across the complete pre-brake/handoff/restart state before choosing the actor
  or transition property to change. Keep all 63 flat gates, both long compositions,
  the five passing surfaces and every physics-quality gate; no broad terrain
  expansion or automatic extension while the original downhill cases fail.
