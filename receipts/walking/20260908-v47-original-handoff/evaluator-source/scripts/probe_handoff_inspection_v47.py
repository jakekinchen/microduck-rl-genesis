"""Frozen V47 original/V46 matched flat and downhill handoff inspection."""
import argparse
import gzip
...
SUITE=ROOT/"experiments/walking/handoff-inspection-v47.json"
FREEZE=ROOT/"experiments/walking/handoff-inspection-freeze-v47.json"
...
SOURCES=list(dict.fromkeys(SOURCES+EXTRA+["scripts/probe_handoff_inspection_v47.py", "experiments/walking/handoff_inspection_v47.py", "experiments/walking/handoff-inspection-v47.json", "experiments/walking/HANDOFF-INSPECTION-v47.md", "experiments/walking/snapshots_v43.py"]))

def main():
    p=argparse.ArgumentParser()
    p.add_argument("--freeze",action="store_true")
    p.add_argument("--pair", choices=("original", "v46"), default="original")
    ...
    if a.pair=="original":
        WALK_RUN="walking-20260906-v21";walk_sha=WALK_SHA
    from experiments.walking.handoff_inspection_v47 import retain_state
    ...
    try:
                        ...
                        for index,window in enumerate(session["windows"]):
                            ...
                            try:
                                for i in range(round(window["duration_s"]*50)):
                                    ...
                                    if index == 0 and i+1 in suite["capture_after_control_steps"]:
                                        retain_state(world, a.output, session["id"], i+1)
                                    if writer and i%2==0:
                                        frame=Image.fromarray(world.walking_frame())
                                        draw=ImageDraw.Draw(frame)
                                        draw.rectangle((0,0,720,38),fill=(15,20,30))
                                        draw.text((8,8),f'V47 | {name} | {row["session_time_s"]:.2f}s | {role}',fill="white")
        ...
        result={"schema":"microduck.terrain-endurance-result/v1","bank":a.bank,"controller_variant":"V30-handoff-inspection-v47","pair":a.pair,"proof_class":"visible_development",
