"""Continuous native walk/brake/stand/restart learning with original controllers."""
...
# Each episode has three 18-second windows and no reset between commands.
COMMANDS = [[.12, 0, 0], [0, 0, .35], [.08, 0, .30],
            [.17, 0, 0], [0, 0, -.35], [.08, 0, -.30]]
BUCKETS = [('flat-a', 0., 0), ('flat-b', 0., 3),
           ('downhill', 3., 0), ('uphill', -3., 0)]


def request_at(step, bucket, episode):
    window, tick = divmod(step, 900)
    if not 50 <= tick < 650:
        return np.zeros(3, np.float32)
    index = (BUCKETS[bucket][2] + window + episode) % 6
    if bucket >= 2:
        # Slopes first learn repeated straight locomotion, not a changed slope
        # orientation disguised as a downhill repetition.
        return np.array([.10 if (window+episode)%2 else .12, 0, 0], np.float32)
    return np.asarray(COMMANDS[index], np.float32)
...
def make_templates(output):
    ...
    for name, angle, _ in BUCKETS:
        ...
        world = TerrainWorld(BASE/'policy.onnx', ROOT/'.workspace/bam', standing_policy=BASE/'standing/policy.onnx',
            ...
            motor_ticks=6, sensor_ticks=1, yaw=0., seed=26090744)
...
class NativeSequenceEnv:
    ...
    def __init__(self, num_envs, seed, output):
        ...
        self.cfg = dict(task='Native-Full-Sequences-v44', backend='native-mujoco',
            actor_observations=61, critic_observations=61, action_filter=False,
            episodes='54 s; three 18 s windows; HOME only on fall or full episode timeout',
        ...
        self.worlds = [clone_world(self.templates[i%4][0]) for i in range(num_envs)]
        self.adapters = [AssignedAction() for _ in self.worlds]
        self.episode_length_buf = torch.zeros(num_envs, dtype=torch.long)
        self.episodes = np.full(num_envs, -1, np.int64)
        self.reset_counts = np.zeros(4, np.int64)
        self.falls = np.zeros(4, np.int64)
        self.completed = np.zeros(4, np.int64)
        self.phase_counts = np.zeros((4, 5), np.int64)
        self.switches = np.zeros((4, 2), np.int64)
        self.last_stand = [True]*num_envs
        self.steps = 0
        self.min_obs = np.full((4, 61), np.inf)
        self.max_obs = np.full((4, 61), -np.inf)
    ...
    def reset_one(self, i):
        w = self.worlds[i]
        restore(w, self.templates[i%4][1])
        reset_controllers(w)
        adapter = self.adapters[i]
        w.walking_policy = w.standing_policy = adapter
        self.episode_length_buf[i] = 0
        self.episodes[i] += 1
        self.reset_counts[i%4] += 1
        self.last_stand[i] = True

    def get_observations(self):
        obs = np.concatenate([preview(w, request_at(int(self.episode_length_buf[i]), i%4, int(self.episodes[i])))
    ...
    def step(self, actions):
        ...
        for i, (w, action) in enumerate(zip(self.worlds, actions)):
            bucket, step = i%4, int(self.episode_length_buf[i])
            ...
            if stand != self.last_stand[i]:
                ...
                history = dict(global_step=self.steps, env=i, bucket=bucket, episode=int(self.episodes[i]),
                    episode_step=step, direction='walk-to-stand' if stand else 'stand-to-walk',
            ...
            cost = posture+stop_cost+2*max(0., internal-.5)+10*(row['minimum_actual_joint_margin_rad']<.04)
    ...
    def coverage(self):
        return dict(control_steps=self.steps, transitions=self.steps*self.num_envs, buckets=[b[0] for b in BUCKETS],
            reset_counts=self.reset_counts.tolist(), falls=self.falls.tolist(), complete_54s_episodes=self.completed.tolist(),
