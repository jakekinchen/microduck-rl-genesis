...
FREEZE=ROOT/"experiments/walking/native-sequence-flat-freeze-v44-r2.json"
...
def main():
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "V44 joint FINAL walker and stander, V16 ramp and fixed V30 heading; exposed regression",
    ...
    if (record["status"] != "completed" or record["variant"] != "native-sequence-walking-v44"
            or record["checkpoint"] != "model_499.pt" or record["new_transitions"] != 768_000
    ...
    stand, stand_parity, stand_export = export_walking("sequence-native-20260907-v44-standing", a.output/"standing")
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V44 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "native-sequence-v44-flat-regression",
              ...
              "controller": "Fixed V16 command slew and V30 float32 correction-headroom persistent-reference filtered IMU feedback; V44 joint FINAL walking / V44 joint FINAL standing; exact-zero ramped-command routing",
