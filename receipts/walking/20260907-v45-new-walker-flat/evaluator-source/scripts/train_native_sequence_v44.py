...
FREEZE=ROOT/'experiments/walking/native-sequence-freeze-v44.json'
PARENTS={'walking':'walking-20260906-v21','standing':'standing-20260906-v15'}
EXTRA=['microduck/sequence_actor_v44.py','microduck/native_sequence_env_v44.py',
       'scripts/train_native_sequence_v44.py','scripts/probe_native_sequence_v44.py',
       'experiments/walking/NATIVE-SEQUENCE-v44.md','experiments/walking/native_candidate_v44.py',
       'scripts/evaluate_native_sequence_flat_v44.py','scripts/evaluate_native_sequence_endurance_v44.py',
       'scripts/evaluate_native_sequence_surfaces_v44.py','scripts/evaluate_native_sequence_fresh_v44.py',
       'experiments/walking/native-sequence-fresh-v44.json','tests/test_sequence_actor_v44.py']


def binding():
    frozen=json.loads((ROOT/'experiments/walking/native-standing-freeze-v42.json').read_text())
    ...
    return dict(schema='microduck.native-sequence-freeze/v44',source_sha256=sources,
        warm_start=warm,seed=26090744,full_transitions=768000,smoke_transitions=960)


def main():
    p=argparse.ArgumentParser();p.add_argument('--freeze',action='store_true');p.add_argument('--run-id')
    p.add_argument('--num-envs',type=int,default=64);p.add_argument('--iterations',type=int,default=500);a=p.parse_args()
    frozen=binding()
    if a.freeze:
        with FREEZE.open('x') as f:json.dump(frozen,f,indent=2);f.write('\n')
        return
    if not a.run_id or not a.run_id.replace('-','').isalnum() or (a.num_envs,a.iterations) not in [(8,5),(64,500)]:p.error('8x5 smoke or 64x500 full')
    if frozen!=json.loads(FREEZE.read_text()):raise ValueError('freeze drift')
    preflight=ROOT/'receipts/walking/20260907-v44-sequence-conformance'
    ...
    if shutil.disk_usage(ROOT).free<2_000_000_000:raise ValueError('less than 2 GB free before bounded run')
    out=ROOT/'logs'/a.run_id;out.mkdir(parents=True,exist_ok=False)
    record=dict(schema='microduck.walking-training/v1',variant='native-sequence-v44',proof_class='first_party_development',
    ...
    try:
        ...
        from microduck.native_sequence_env_v44 import NativeSequenceEnv
        torch.set_num_threads(1);torch.manual_seed(26090744);np.random.seed(26090744)
        cfg=copy.deepcopy(TRAIN_CFG);cfg.update(seed=26090744,run_name=a.run_id,save_interval=500)
        cfg['algorithm'].update(learning_rate=3e-5,schedule='fixed',entropy_coef=0.,gamma=.999)
        cfg['obs_groups']['critic']=['policy']
        cfg['actor']['class_name']='microduck.sequence_actor_v44:SequenceActor'
        cfg['actor']['distribution_cfg']['std_range']=[.005,.15]
        record['train_cfg']=copy.deepcopy(cfg)
        env=NativeSequenceEnv(a.num_envs,26090744,out);record['env_cfg']=env.cfg
    ...
    for role in PARENTS:
        ...
        component={**record,'variant':'native-sequence-'+role+'-v44','joint_checkpoint_sha256':record['checkpoint_sha256']}
