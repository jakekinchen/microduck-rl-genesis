"""V22 new physical-combination bank with exact frozen V21/V15 actors."""
import argparse
import json
...
from scripts.evaluate_walking_yaw_refinement import SOURCES as PRIOR_SOURCES

SOURCES = list(dict.fromkeys(PRIOR_SOURCES + ["scripts/evaluate_walking_physical_development.py", "experiments/walking/physical_domain.py", "tests/test_walking_physical_domain.py", "experiments/walking/physical-development-v22.json", "experiments/walking/PHYSICAL-DEVELOPMENT-v22.md", "scripts/evaluate_walking_yaw_refinement.py", "scripts/train_walking_yaw_refinement.py", "microduck/walking_yaw_refinement_env.py", "tests/test_walking_yaw_refinement.py", "experiments/walking/YAW-REFINEMENT-v21.md", "microduck/filtered_heading_servo.py", "experiments/walking/filtered_heading_world.py", "tests/test_filtered_heading.py", "scripts/evaluate_walking_unbraced.py",
...
FREEZE = ROOT/"experiments/walking/physical-development-freeze-v22.json"
STAND_SHA = "acab8402e262dbb6af5a3fab9b67fa4ce5e34a4d23cadb127fe6dfa475a70e46"
WALK_SHA = "7b5e166c13a8086fdd21b5ad237a0e69aa75828ad6f8fb5536f3ea057535a322"


def validate_regressions():
    from scripts.evaluate_walking_heading import verify_input_manifest
    for name, count in (("20260906-v21-current",21),("20260906-v21-exposed-repeated",42)):
        folder = ROOT/"receipts/walking"/name
        verify_input_manifest(folder)
        record = json.loads((folder/"evaluation.json").read_text())
        if (record["passed_cases"] != count or record["total_cases"] != count
                or not all(c["passed"] and not c["failures"] for c in record["case_reports"])):
            raise ValueError("all 63 original/exposed gates must pass before this bank")
...
def main():
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "Fixed V21/V15 final actors, V16 ramp/V19 heading; new physical development after all 63 exposed gates passed",
                       "walking_checkpoint_sha256": WALK_SHA,
                       "input_manifests": {n: digest(ROOT/"receipts/walking"/n/"SHA256SUMS") for n in ("20260906-v21-current", "20260906-v21-exposed-repeated")},
    ...
    for name, sha in frozen["input_manifests"].items():
        if digest(ROOT/"receipts/walking"/name/"SHA256SUMS") != sha:
            raise ValueError("accepted regression receipt changed")
    ...
    if (record["status"] != "completed" or record["variant"] != "walking-v21"
            or record["checkpoint_sha256"] != WALK_SHA
    ...
    suite_path = ROOT/"experiments/walking/physical-development-v22.json"
    import numpy as np
    import torch
    import imageio.v2 as imageio
    from PIL import Image, ImageDraw
    from scripts.export_walking import export_walking
    from experiments.walking.physical_domain import PhysicalDevelopmentWorld as StandingSwitchWalkingWorld
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                world = StandingSwitchWalkingWorld(walk, ROOT/".workspace/bam", standing_policy=stand,
                    model_directory=model_dir, domain=timing["domain"], motor_ticks=timing["motor_ticks"], sensor_ticks=timing["sensor_ticks"],
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V22 DOMAIN | V21 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                                ...
                                report.update(case_id=name, timing_profile=timing["id"], repeated_window=repeat+1, session_window_start_s=offset, materialized_domain=world.materialized_domain, observed_contact_samples=world.checked_contact_samples)
                    ...
                    world.close()
    result = {"schema": "microduck.walking-physical-development/v1", "acceptance_variant": "new-physical-combinations-v22",
              "proof_class": "visible_development", "held_out": False, "fresh_development": True, "exposed_repeated_bank": False,
              ...
              "controller": "Fixed V16 command slew and V19 filtered IMU feedback; V21 FINAL walking / V15 FINAL standing; exact-zero ramped-command routing",
              "model_scope": {"variant": "complete-contact-v11-with-materialized-v22-domain", "scene_sha256": digest(model_dir/"scene.xml"),
                              "robot_sha256": digest(model_dir/"robot.xml"), "old_reduced_model_acceptance": False},
              "boundary": "New exposed flat-floor physical combinations. Explicit per-case materialized mass/inertia/friction; original V21 scores unchanged. All component gates and continuous histories retained. Not unseen terrain families, calibrated physics or physical acceptance."}
    for name, sha in frozen["source_sha256"].items():
        if digest(ROOT/name) != sha:
