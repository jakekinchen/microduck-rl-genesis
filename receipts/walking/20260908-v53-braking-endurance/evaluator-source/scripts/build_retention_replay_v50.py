...
out=ROOT/'experiments/walking/retention-replay-v50'
if __name__=='__main__':
    out.mkdir(exist_ok=False)
    inputs=[];targets=[];reports=[]
    for name in ['20260906-v30-flat-regression','20260906-v30-repeated-regression-r2','20260906-v30-heading-endurance','20260906-v30-surface-baseline']:
        ...
        with opener(path,'rt') as f:
            for line in f:
                ...
                if row['actor_mode']!='walking' or case not in allowed:continue
                obs=np.asarray(row['actor_observation'],np.float32)
                if not np.any(obs[48:51]) or not np.isfinite(obs).all():raise ValueError('invalid teacher input')
        ...
        reports.append(dict(receipt=name,rows=count,allowed_cases=sorted(allowed),manifest_sha256=digest(folder/'SHA256SUMS'),trace_sha256=digest(path)))
