...
SOURCES=["scripts/evaluate_walking_posture.py","scripts/evaluate_walking.py","experiments/walking/posture.py","microduck/constants.py","scripts/export_walking.py","experiments/walking/world.py",
...
def main():
    ...
    a=p.parse_args()
    frozen=ROOT/"experiments/walking/evaluator-freeze-posture-v1.json"
    if a.freeze:
        ...
        frozen.write_text(json.dumps({"schema":"microduck.walking-evaluator-freeze/v1","source_sha256":{s:digest(ROOT/s) for s in SOURCES},
                                     "candidate":"declared final walking-v3 checkpoint, not yet inspected", "held_out":False},indent=2)+"\n")
