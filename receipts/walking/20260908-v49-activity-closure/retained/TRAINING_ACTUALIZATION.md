...
- [ ] Freeze a walking-only correction for downhill yaw oscillation, the earliest
  remaining scored failure (.244/.241 rad/s before braking, versus .20). Keep
  original V15 standing and V30 command control fixed; retain 63 flat cases,
  both full 180-second compositions, the five original passing surfaces and
  every gait/contact/actuator gate. Evaluate full sequences; a passing walking
  prefix alone is not promotion. Do not replace absolute yaw error with a
  signed-average score or weaken thresholds.
- [ ] After the walking correction, use the three clean V49 stopping witnesses
  to develop a repeatable state-conditioned braking transition. Preserve V15
  steady standing and test successive stops, restarts and different gait phases;
