...
class WorkspaceTests(unittest.TestCase):
    ...
    def test_nested_recovery_requires_complete_manifested_envelope(self):
        parent = self.root / "receipts/walking/recovery"
        folder = parent / "state--original"
        folder.mkdir(parents=True)
        branch = {"state_id": "state", "anchor": "original", "repeat_exact": True,
                  "passed": False, "case_reports": []}
        (folder / "evaluation.json").write_text(json.dumps(branch))
        for name in ("trajectory.jsonl.gz", "actions-float32.npy", "observations-float32.npy"):
            (folder / name).write_bytes(b"fixture")
        frozen = {"schema": "microduck.recovery-freeze/v49",
                  "parameters": {"schema": "microduck.recovery-diagnostic/v49"}}
        (parent / "freeze.json").write_text(json.dumps(frozen))
        result = {"phase": "compare", "complete": True, "branches": [branch],
                  "freeze_sha256": hashlib.sha256((parent / "freeze.json").read_bytes()).hexdigest()}
        (parent / "result.json").write_text(json.dumps(result))
        self.manifest(parent)
        runs, errors = self.inspector.evaluations(str(folder.relative_to(self.root)))
        self.assertEqual(errors, [])
        self.assertEqual(runs[0]["proof_class"], "counterfactual_diagnostic")
        self.assertIsNone(runs[0]["passed"])
        self.assertFalse(runs[0]["supported_metrics"])
        # A child manifest must not bypass the authoritative parent's hashes.
        (folder / "trajectory.jsonl.gz").write_bytes(b"changed")
        self.manifest(folder)
        runs, errors = self.inspector.evaluations(str(folder.relative_to(self.root)))
        self.assertEqual(runs, [])
        self.assertIn("manifest", errors[0]["error"])
        # A matching filename or newly generated manifest cannot excuse a
        # branch that disagrees with the complete parent result.
        branch["passed"] = True
        (folder / "evaluation.json").write_text(json.dumps(branch))
        self.manifest(parent)
        runs, errors = self.inspector.evaluations(str(folder.relative_to(self.root)))
        self.assertEqual(runs, [])
        self.assertIn("diagnostic envelope", errors[0]["error"])
    ...
    def test_heading_variants_preserve_load_rejection_and_standing_identity(self):
        ...
        for variant in ["yaw-refinement-filtered-heading-v21", "heading-persistence-v24-flat-regression",
                        ...
                        "native-standing-retention-v48-flat-regression"]:
