"""V50 walking FINAL and original V15 standing on unchanged fresh gates."""
import argparse
import gzip
...
SUITE=ROOT/"experiments/walking/native-sequence-fresh-v44.json"
FREEZE=ROOT/"experiments/walking/native-retention-fresh-freeze-v50.json"
...
SOURCES=list(dict.fromkeys(V23_SOURCES+[
    ...
    "tests/test_persistent_heading_v24.py", "experiments/walking/native-sequence-fresh-v44.json",
...
SOURCES=list(dict.fromkeys(SOURCES+['microduck/heading_headroom_v30.py', 'tests/test_heading_float32_v30.py', 'experiments/walking/HEADING-FLOAT32-v30.md', 'scripts/evaluate_heading_headroom_endurance_v29.py', 'scripts/evaluate_heading_headroom_endurance_v30.py']))
...
SOURCES = list(dict.fromkeys(SOURCES + ["scripts/evaluate_native_sequence_endurance_v44.py", "scripts/train_native_sequence_v44.py", "microduck/native_sequence_env_v44.py", "experiments/walking/NATIVE-SEQUENCE-v44.md", "experiments/walking/native_candidate_v44.py"]))

from scripts.train_native_retention_v50 import EXTRA
SOURCES=list(dict.fromkeys(SOURCES+EXTRA+["scripts/evaluate_native_retention_fresh_v50.py"]))

def main():
    p=argparse.ArgumentParser()
    p.add_argument("--freeze",action="store_true")
    p.add_argument("--bank",choices=("fresh-sequence",))
    ...
    for suffix,expected in [('flat',21),('repeated',42),('endurance',4)]:
        folder=ROOT/f'receipts/walking/20260908-v50-retention-{suffix}'
        from scripts.verify_native_retention_v50 import manifest
        manifest(folder)
        report=json.loads((folder/('probe.json' if suffix=='endurance' else 'evaluation.json')).read_text())
        assert report.get('passed_sessions',report.get('passed_cases'))==expected
        assert all(c['passed'] for c in report['case_reports'])
    folder=ROOT/'receipts/walking/20260908-v50-retention-surfaces'
    manifest(folder)
    surfaces=json.loads((folder/'probe.json').read_text())
    required={'rigid-control-start-1','rigid-control-start-2','soft-low-traction-start-1','soft-low-traction-start-2','unseen-low-medium-start-1'}
    assert surfaces['complete'] and required <= {c['session_id'] for c in surfaces['session_reports'] if c['passed']}
    ...
    walk,wp,walk_export=export_walking(WALK_RUN,a.output)
    stand,sp,stand_export=export_walking(STAND_RUN,a.output/"standing")
    ...
    try:
        ...
        result={"schema":"microduck.terrain-endurance-result/v1","bank":a.bank,"controller_variant":"V30-with-native-retention-v50","proof_class":"visible_development",
