"""V30: same bounded headroom with the original float32 command arithmetic."""
import math
import numpy as np
...
class Float32HeadingHeadroomServo(MotionHeadingServo):
    def step(self, requested_twist, orientation_wxyz, dt=.02):
        ...
        if control['mode'] != 'stopped':
            ...
            constrained = requested.copy()
            constrained[2] = np.clip(requested_yaw+raw,-POLICY_YAW_LIMIT_RAD_S,POLICY_YAW_LIMIT_RAD_S)
            raw = float(constrained[2]-requested[2])
