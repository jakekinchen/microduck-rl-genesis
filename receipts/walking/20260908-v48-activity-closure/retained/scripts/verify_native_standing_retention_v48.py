"""Offline V48 source, full recorded learning data and fixed-walker lineage verification."""
import argparse
import gzip
import hashlib
import json
...
def main():
    ...
    from scripts.train_native_standing_retention_v48 import binding,FREEZE,PARENTS
    from experiments.walking.native_candidate_v48 import candidate_sha,RUN
    assert binding()==json.loads(FREEZE.read_text())
    walk_sha,stand_sha=candidate_sha()
    frozen_counts={}
    for path in sorted((ROOT/'experiments/walking').glob('native-standing-retention*freeze-v48.json')):
    ...
    for role,parent_run in PARENTS.items():
        ...
        if role=='standing' and not any(changes.values()):raise ValueError('stander did not learn')
        if role=='walking':
            for key,value in state.items():
                if value.numpy().tobytes()!=parent_state[key].numpy().tobytes():raise ValueError('fixed walking tensor changed')
        if role=='standing':
    ...
    if scalar_count < 250: raise ValueError("missing learning scalar coverage")
    receipts={}
    for name in ['20260908-v48-conformance','20260908-v48-standing-retention-flat',
                 '20260908-v48-standing-retention-repeated','20260908-v48-standing-retention-endurance','20260908-v48-standing-retention-surfaces',
                 '20260908-v47-handoff-verification','20260908-v48-replay-parity']:
    ...
    shutil.copy2(Path(__file__),a.output/'verify_native_standing_retention_v48.py')
