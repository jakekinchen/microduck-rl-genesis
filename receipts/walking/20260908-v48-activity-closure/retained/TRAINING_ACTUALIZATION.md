...
- [ ] Freeze a bounded recovery-feasibility diagnostic at the original downhill
  pre-brake/handoff and V48 late-composition failure. Reproduce complete state
  histories; compare original/V48 standing from identical inputs and search
  for a safe recovery under unchanged actuator, timing and body-contact gates.
  Optimized actions are a separate privileged diagnostic, not deployed policy
  success; no solution within a finite budget does not prove impossibility.
  Before another learning correction, define closed-loop retention through
  180 seconds and keep all 63 flat gates plus five original passing surfaces.
  Fresh/protected banks remain closed while downhill prerequisites fail.
