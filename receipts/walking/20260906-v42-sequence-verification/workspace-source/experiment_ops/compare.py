...
def bounded_trace_lines(path: Path, max_bytes: int):
    """Bound decoded bytes and individual rows, including compressed traces."""
    ...
    try:
        ...
        raise ValueError("incomplete compressed trace") from error


def load_trace(path: Path, action_width=14, *, max_bytes=MAX_BYTES, max_rows=MAX_ROWS):
    if type(max_bytes) is not int or not 1 <= max_bytes <= 512 * 1024 * 1024:
        raise ValueError("trace budget must be an integer <= 512 MiB")
    if type(max_rows) is not int or not 1 <= max_rows <= 50000:
        raise ValueError("trace row budget must be an integer <= 50000")
    ...
    for line, raw in enumerate(bounded_trace_lines(path, max_bytes), 1):
        consumed += len(raw)
        digest.update(raw)
        if not raw.strip():
    ...
    if path.name.endswith(".jsonl.gz"):
        # Manifests bind stored bytes, while the parsing budget binds decoded bytes.
        digest, source_bytes = hashlib.sha256(), 0
        with path.open("rb") as stream:
            while raw := stream.read(128 * 1024):
    ...
    return cases, {"path": str(path), "sha256": digest.hexdigest(), "bytes": source_bytes, "rows": rows,
                   **({"decoded_bytes": consumed} if path.name.endswith(".jsonl.gz") else {})}
