# V42 native standing rejected; V30 remains retained

Read `experiments/walking/SEQUENCE-RESULTS-v41.md`. The bounded contact/stopping
sequence is complete through failed prerequisite gates. V33/V34 correct the
sampled collision-hull import mismatch, but eight fixed-action probes do not
close softer robot-contact agreement. V38, V41 and V42 are rejected.

V42 preserves all 63 original/repeated flat gates. It passes only 1/4 diagnostic
sessions: both downhill starts fall and one long composition falls at 140.16 s.
Existing exposed surfaces regress to 3/14 sessions. Keep original V21/V15 actors
with V30, whose limited flat/endurance results remain unchanged. Fresh terrain
and protected final banks remain unopened; public priors are not physical
calibration.

The active packet binds a passing V42 flat reference and its full source and
paired-policy receipt. It explicitly does not represent an endurance/surface
pass or promoted policy. Failure videos and raw traces are linked in the report.
Compressed traces are now supported by bounded, manifest-checked inspection.
All jobs from this sequence are complete; check live activity before any launch.

Next quantify coverage of candidate-induced handoff states and delay histories.
The native training bank used V30-generated starts and isolated three-second
episodes; evaluation composes the new stander with walking and repeated stops.
Treat this as a hypothesis requiring measurement, then freeze one intervention.
