# V30 heading verified; surface and physical gaps remain

Read `experiments/walking/PUBLIC-SURFACES-RESULTS.md`. V30 with original V21/V15
actors passes all 63 flat gates and both 180-second compositions; endpoint
heading error is 6.24/8.95 degrees. Surface admission remains 5/14 and downhill
0/2. The three newly trained motor policies are not promoted. Public data was
collected without owner measurements, but matching Ducky calibration remains
unestablished. Robot-level fixed-action agreement is only 2/5 under its frozen
limits, despite passive primitive agreement.

The active packet now binds a V30 flat reference and its exact controller/source
receipt. The long-composition video is a separate recorded-pose visualization
linked from the results report. Next match support/contact representations and
inspect first divergence, then repair standing transitions. All jobs from this
pass are complete; check live activity before a new launch. The disk-error
partial run and source-identical successful retry remain separately retained.
