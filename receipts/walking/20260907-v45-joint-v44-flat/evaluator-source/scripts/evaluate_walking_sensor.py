"""Additive neutral-head rejection on the unchanged command/timing battery."""
import argparse
import json
...
SOURCES=["scripts/evaluate_walking_sensor.py","scripts/evaluate_walking.py","experiments/walking/posture.py","microduck/constants.py","scripts/export_walking.py","experiments/walking/world.py","experiments/walking/sensor_world.py",
         "experiments/walking/metrics.py","experiments/walking/tracking-suite-v1.json","experiments/laser/gait.py",
...
def main():
    ...
    a=p.parse_args()
    frozen=ROOT/"experiments/walking/evaluator-freeze-sensor-v1.json"
    if a.freeze:
        ...
        frozen.write_text(json.dumps({"schema":"microduck.walking-evaluator-freeze/v1","source_sha256":{s:digest(ROOT/s) for s in SOURCES},
                                     "candidate":"declared final walking-v5 checkpoint, not yet inspected", "held_out":False},indent=2)+"\n")
    ...
    with (a.output/"trajectory.jsonl").open("w") as stream:
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                world=ConsistentSensorWalkingWorld(policy,ROOT/".workspace/bam",motor_ticks=timing["motor_ticks"],sensor_ticks=timing["sensor_ticks"],yaw=case["yaw"],seed=suite["seed"],render=a.video)
                ...
                try:
                    ...
                    world.close()
    result={"schema":"microduck.walking-evaluation/v1","acceptance_variant":"motor-plus-neutral-head-v1","proof_class":"visible_development","held_out":False,
            ...
            "physical_transfer_validated":False,"reserved_opened":False,"sensor_sampling":"copied-current-state-v1",
            "boundary":"Clock-consistent command/timing plus neutral-head visible development. IMU sampled on copied current state; no implicit extra physics-tick delay or force mutation. Same numerical gates and raw ONNX actions. No physical calibration; old timing regressions retained separately."}
