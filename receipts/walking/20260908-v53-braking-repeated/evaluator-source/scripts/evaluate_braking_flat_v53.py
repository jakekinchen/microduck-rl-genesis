"""V50 joint walking and standing FINAL with V30 controller on exposed flat gates."""
import argparse
import gzip
import json
...
FREEZE=ROOT/"experiments/walking/braking-flat-freeze-v53.json"
...
SOURCES=list(dict.fromkeys(SOURCES+['microduck/heading_headroom_v30.py', 'tests/test_heading_float32_v30.py', 'experiments/walking/HEADING-FLOAT32-v30.md', 'scripts/evaluate_heading_headroom_flat_v29.py', 'scripts/evaluate_heading_headroom_flat_v30.py']))


SOURCES = list(dict.fromkeys(SOURCES + ["scripts/evaluate_native_sequence_flat_v44.py", "scripts/train_native_sequence_v44.py", "microduck/native_sequence_env_v44.py", "experiments/walking/NATIVE-SEQUENCE-v44.md", "experiments/walking/native_candidate_v44.py"]))

from scripts.train_native_retention_v50 import EXTRA
SOURCES=list(dict.fromkeys(SOURCES+EXTRA+["scripts/evaluate_native_sequence_flat_v44_r2.py"]))
...
def main():
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "V53 retention braker with V50 walker/V15 stander, V16 ramp and V30 heading; exposed regression",
    ...
    if (record["status"] != "completed" or record["variant"] != "native-retention-walking-v50"
            or record["checkpoint"] != "model_749.pt" or record["new_transitions"] != 864_000
            or record["args"]["num_envs"] != 48 or record["args"]["iterations"] != 750):
        raise ValueError("exact bounded V50 component FINAL required")
    ...
    stand, stand_parity, stand_export = export_walking(STAND_RUN, a.output/"standing")
    ...
    if record["checkpoint_sha256"] != walk_sha: raise ValueError("joint walking component mismatch")
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V53 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "braking-v53-flat-regression",
              "proof_class": "visible_development", "held_out": False, "fresh_development": False, "exposed_repeated_bank": a.fresh,
              "braking_policy_sha256": digest(BRAKE_RUN/"policy.onnx"), "policy_sha256": digest(walk), "standing_policy_sha256": digest(stand), "standing_policy_used": True,
              ...
              "controller": "Fixed V16 command slew and V30 float32 correction-headroom persistent-reference filtered IMU feedback; V50 corrected walker / original V15 standing plus V53 transient braking; exact-zero ramped-command routing",
