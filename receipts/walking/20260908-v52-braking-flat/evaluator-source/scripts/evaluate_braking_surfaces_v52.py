"""V50 walking FINAL and original V15 standing on unchanged surfaces gates."""
import argparse
import gzip
...
SUITE=ROOT/"experiments/walking/public-surfaces-v25.json"
FREEZE=ROOT/"experiments/walking/braking-surfaces-freeze-v52.json"
from scripts.evaluate_walking_heading_persistence import SOURCES as V24_SOURCES
SOURCES=list(dict.fromkeys(V24_SOURCES+[
 "scripts/evaluate_walking_public_surfaces.py", "scripts/train_public_surfaces.py",
 "scripts/audit_public_surface_physics.py", "experiments/walking/public_surface_v25.py",
 "microduck/public_surface_env_v25.py", "experiments/walking/PUBLIC-SURFACES-v25.md",
 "experiments/walking/public-surfaces-v25.json", "experiments/walking/public-surfaces-v1/evidence.json"
]))

SOURCES=list(dict.fromkeys(SOURCES+['microduck/heading_headroom_v29.py', 'tests/test_heading_headroom_v29.py', 'experiments/walking/HEADING-HEADROOM-v29.md', 'scripts/evaluate_heading_headroom_surfaces_v29.py']))

SOURCES=list(dict.fromkeys(SOURCES+['microduck/heading_headroom_v30.py', 'tests/test_heading_float32_v30.py', 'experiments/walking/HEADING-FLOAT32-v30.md', 'scripts/evaluate_heading_headroom_surfaces_v29.py', 'scripts/evaluate_heading_headroom_surfaces_v30.py']))
...
SOURCES = list(dict.fromkeys(SOURCES + ["scripts/evaluate_native_sequence_surfaces_v44.py", "scripts/train_native_sequence_v44.py", "microduck/native_sequence_env_v44.py", "experiments/walking/NATIVE-SEQUENCE-v44.md", "experiments/walking/native_candidate_v44.py"]))

from scripts.train_native_retention_v50 import EXTRA
SOURCES=list(dict.fromkeys(SOURCES+EXTRA+["scripts/evaluate_native_sequence_surfaces_v44_r2.py"]))

from scripts.train_braking_v52 import EXTRA as BRAKE_SOURCES
SOURCES=list(dict.fromkeys(SOURCES+BRAKE_SOURCES))

def main():
    p=argparse.ArgumentParser()
    p.add_argument("--freeze",action="store_true")
    p.add_argument("--bank",choices=("surface",))
    p.add_argument("--output",type=Path)
    p.add_argument("--video",action="store_true")
    p.add_argument("--candidate",choices=("native-sequence",),default="native-sequence")
    a=p.parse_args()
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema":"microduck.terrain-endurance-freeze/v1",
                       ...
                       "walking_parent_checkpoint_sha256":WALK_SHA,"standing_parent_checkpoint_sha256":STAND_SHA,
    ...
    from experiments.walking.native_candidate_v50 import candidate_sha, WALK_RUN, STAND_RUN
    walk_sha,stand_sha=candidate_sha()
    from experiments.walking.terrain_v23 import TerrainGaitProbe,materialize_terrain
    from experiments.walking.public_surface_v25 import PublicSurfaceWorld
    ...
    from microduck.braking_v52 import install,retain,RUN as BRAKE_RUN
    torch.set_num_threads(1)
    suite=json.loads(SUITE.read_text())
    sessions=[s for s in suite["sessions"] if s["bank"]==a.bank]
    a.output.mkdir(parents=True,exist_ok=False)
    (a.output/"standing").mkdir()
    walking_run=WALK_RUN
    standing_run=STAND_RUN
    walk,wp,walk_export=export_walking(walking_run,a.output)
    stand,sp,stand_export=export_walking(standing_run,a.output/"standing")
    for run,path,original_sha in ((walking_run,a.output/"training.json",walk_sha),(standing_run,a.output/"standing/training.json",stand_sha)):
        record=json.loads(path.read_text())
        if run.endswith('v25'):
            if record['args']['iterations']!=250 or record['checkpoint']!='model_249.pt' or record['new_transitions']!=6144000:
                raise ValueError('predeclared full-run FINAL required')
        elif record['checkpoint_sha256']!=original_sha: raise ValueError('original actor identity mismatch')
    ...
    exception=None
    try:
        with gzip.open(a.output/"trajectory.jsonl.gz","wt") as stream:
            for session in sessions:
                scene=materialize_terrain(a.output/"terrain-models"/session["id"],session["terrain"])
                world=PublicSurfaceWorld(walk,ROOT/".workspace/bam",standing_policy=stand,model_directory=model_dir,
                    terrain_scene=scene,profile=session["profile"],domain=session["domain"],motor_ticks=suite["motor_ticks"],sensor_ticks=suite["sensor_ticks"],
                ...
                try:
                    with record_self_loads(world) as loads:
                            ...
                            try:
                                for i in range(round(window["duration_s"]*50)):
                                    ...
                                    row.update(case_id=name,session_id=session["id"],session_time_s=row["time_s"],time_s=(i+1)*.02,
                                        actor_observation=world.last_observation[0].tolist(),braking_active=world.brake_v52.active,braking_age=world.brake_v52.age,
                                    ...
                                    expected += world.brake_v52.delta(world.last_observation)[0]
                                    ...
                                    if writer and i%2==0:
                                        ...
                                        draw.text((8,8),f'V50 | {name} | {row["session_time_s"]:.2f}s | {role}',fill="white")
                        ...
                        session_reports.append({"session_id":session["id"],"bucket":session["bucket"],
                            ...
                            "materialized_surface":world.materialized_surface,"materialized_domain":world.materialized_domain,"materialized_terrain":world.materialized_terrain,
        ...
        result={"schema":"microduck.terrain-endurance-result/v1","bank":a.bank,"controller_variant":"V52-transient-braking-on-V50-V15","candidate":a.candidate,"proof_class":"visible_development",
                ...
                "braking_policy_sha256":digest(BRAKE_RUN/"policy.onnx"),"policy_sha256":digest(walk),"standing_policy_sha256":digest(stand),"evaluator_freeze_sha256":digest(FREEZE),
