"""V44 native standing FINAL with V30 controller on exposed endurance gates."""
import argparse
import gzip
...
FREEZE=ROOT/"experiments/walking/component-endurance-freeze-v45.json"
...
def main():
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            ...
            f.write("\n")
        return
    if not a.pair: p.error("exact declared pair required")
    ...
    try:
                            ...
                            try:
                                for i in range(round(window["duration_s"]*50)):
                                    ...
                                    if writer and i%2==0:
                                        ...
                                        draw.text((8,8),f'V45 | {name} | {row["session_time_s"]:.2f}s | {role}',fill="white")
        ...
        result={"schema":"microduck.terrain-endurance-result/v1","bank":a.bank,"controller_variant":"V30-component-isolation-v45","pair":a.pair,"proof_class":"visible_development",
