"""V44 joint walking and standing FINAL with V30 controller on exposed flat gates."""
import argparse
import gzip
import json
...
FREEZE=ROOT/"experiments/walking/component-flat-freeze-v45.json"
...
from experiments.walking.component_pairs_v45 import PAIRS, FLAT_CASES, resolve_pair
SOURCES=list(dict.fromkeys(SOURCES+["experiments/walking/component_pairs_v45.py", "experiments/walking/COMPONENT-ISOLATION-v45.md", "scripts/evaluate_component_flat_v45.py", "scripts/evaluate_component_endurance_v45.py"]))
...
def main():
    ...
    p.add_argument("--pair",choices=tuple(PAIRS))
    a = p.parse_args()
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "Four exact original/V44 component pairs; two exposed flat failures; V16/V30 fixed",
                       "source_sha256": {name: digest(ROOT/name) for name in SOURCES}}, f, indent=2)
            f.write("\n")
        return
    if a.fresh: p.error("V45 uses only its two frozen original flat cases")
    if a.output is None or not a.pair: p.error('pair and fresh output required')
    frozen=json.loads(FREEZE.read_text())
    for name,sha in frozen['source_sha256'].items():
        if digest(ROOT/name)!=sha: raise ValueError('frozen source drift: '+name)
    (walk_run,stand_run),(walk_sha,stand_sha)=resolve_pair(a.pair)
    record=json.loads((ROOT/'logs'/walk_run/'run.json').read_text())
    ...
    walk, walk_parity, walk_export = export_walking(walk_run, a.output)
    ...
    with gzip.open(a.output/"trajectory.jsonl.gz","wt") as stream:
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                if base not in FLAT_CASES: continue
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V45 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "component-isolation-v45-flat", "pair": a.pair,
              ...
              "controller": "Fixed V16 command slew and V30 float32 correction-headroom persistent-reference filtered IMU feedback; V45 declared component pair; exact-zero ramped-command routing",
