...
FREEZE=ROOT/"experiments/walking/native-retention-flat-freeze-v46.json"
...
def main():
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "V46 FINAL walker and original V15 stander, V16 ramp and fixed V30 heading; exposed regression",
    ...
    if (record["status"] != "completed" or record["variant"] != "native-retention-walking-v46"
            or record["checkpoint"] != "model_249.pt" or record["new_transitions"] != 360_000
            or record["args"]["num_envs"] != 60 or record["args"]["iterations"] != 250):
        raise ValueError("exact bounded V44 component FINAL required")
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V46 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "native-retention-v46-flat-regression",
              ...
              "controller": "Fixed V16 command slew and V30 float32 correction-headroom persistent-reference filtered IMU feedback; V46 corrected walker / original V15 standing; exact-zero ramped-command routing",
