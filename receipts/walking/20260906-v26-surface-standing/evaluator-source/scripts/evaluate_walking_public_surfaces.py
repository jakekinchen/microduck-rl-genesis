"""Preregistered V25 native terrain/endurance with exact V21/V15 components."""
import argparse
...
FREEZE=ROOT/"experiments/walking/public-surfaces-freeze-v25-r4.json"
...
def main():
    ...
    p.add_argument("--candidate",choices=("baseline","walking","paired"),default="baseline")
    a=p.parse_args()
    ...
    try:
        with (a.output/"trajectory.jsonl").open("w") as stream:
            for session in sessions:
                ...
                world.heading_servo = PersistentHeadingServo()
                full_rows = []
                            ...
                            try:
                                for i in range(round(window["duration_s"]*50)):
                                    ...
                                    if writer and i%2==0:
                                        ...
                                        draw.text((8,8),f'V25 | {name} | {row["session_time_s"]:.2f}s | {role}',fill="white")
