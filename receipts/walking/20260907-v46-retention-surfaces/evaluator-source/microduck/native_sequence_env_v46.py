...
# Twelve fixed, balanced template/timing cells; actual application is audited.
from microduck.native_sequence_env_v44 import BUCKETS as OLD_BUCKETS, request_at as old_request
TIMINGS=[(4,1),(0,0),(6,1)]
BUCKETS=[(f'{name}-motor{motor}-sensor{sensor}',angle,offset,motor,sensor)
         for name,angle,offset in OLD_BUCKETS for motor,sensor in TIMINGS]


def request_at(step,bucket,episode):
    return old_request(step,bucket//3,episode)
...
def make_templates(output):
    templates = []
    for name, angle, _, motor, sensor in BUCKETS:
        ...
        world = TerrainWorld(BASE/'policy.onnx', ROOT/'.workspace/bam', standing_policy=BASE/'standing/policy.onnx',
            ...
            motor_ticks=motor, sensor_ticks=sensor, yaw=0., seed=26090746)
...
class NativeSequenceEnv:
    ...
    def __init__(self, num_envs, seed, output):
        ...
        self.cfg = dict(task='Native-Retained-Walker-v46', backend='native-mujoco',
            ...
            timings=TIMINGS, standing_actor_frozen=True)
        self.templates = make_templates(output)
        self.worlds = [clone_world(self.templates[i%12][0]) for i in range(num_envs)]
        self.adapters = [AssignedAction() for _ in self.worlds]
        self.episode_length_buf = torch.zeros(num_envs, dtype=torch.long)
        self.episodes = np.full(num_envs, -1, np.int64)
        self.reset_counts = np.zeros(12, np.int64)
        self.falls = np.zeros(12, np.int64)
        self.completed = np.zeros(12, np.int64)
        self.phase_counts = np.zeros((12, 5), np.int64)
        self.switches = np.zeros((12, 2), np.int64)
        self.last_stand = [True]*num_envs
        self.steps = 0
        self.min_obs = np.full((12, 61), np.inf)
        self.max_obs = np.full((12, 61), -np.inf)
    ...
    def reset_one(self, i):
        w = self.worlds[i]
        restore(w, self.templates[i%12][1])
        reset_controllers(w)
        adapter = self.adapters[i]
        w.walking_policy = w.standing_policy = adapter
        self.episode_length_buf[i] = 0
        self.episodes[i] += 1
        self.reset_counts[i%12] += 1
        self.last_stand[i] = True

    def get_observations(self):
        obs = np.concatenate([preview(w, request_at(int(self.episode_length_buf[i]), i%12, int(self.episodes[i])))
    ...
    def step(self, actions):
        ...
        for i, (w, action) in enumerate(zip(self.worlds, actions)):
            bucket, step = i%12, int(self.episode_length_buf[i])
