"""V48 standing FINAL with original V21 walker with V30 controller on exposed flat gates."""
import argparse
import gzip
import json
...
FREEZE=ROOT/"experiments/walking/native-standing-retention-flat-freeze-v48.json"
...
def main():
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "Original V21 walker and V48 FINAL stander, V16 ramp and fixed V30 heading; exposed regression",
    ...
    if a.run_id != WALK_RUN:raise ValueError('original V21 walker required')
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V48 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "native-standing-retention-v48-flat-regression",
              ...
              "controller": "Fixed V16 command slew and V30 float32 correction-headroom persistent-reference filtered IMU feedback; Original V21 walking / V48 corrected standing; exact-zero ramped-command routing",
