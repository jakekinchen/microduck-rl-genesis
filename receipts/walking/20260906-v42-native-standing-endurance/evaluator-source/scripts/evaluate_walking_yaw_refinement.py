"""V21 yaw refinement, fixed V15 stander and V19 controller; unchanged gates."""
import argparse
import json
...
FREEZE = ROOT/"experiments/walking/evaluator-freeze-yaw-v21.json"
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "V21 FINAL walker, V15 FINAL stander, V16 ramp and V19 heading; training not yet started",
        ...
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V21 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "yaw-refinement-filtered-heading-v21",
