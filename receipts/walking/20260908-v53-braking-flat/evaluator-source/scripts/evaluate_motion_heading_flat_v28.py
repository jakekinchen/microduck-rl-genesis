"""V28 motion-established heading on the exposed flat regression."""
import argparse
import json
...
FREEZE = ROOT/"experiments/walking/motion-heading-flat-freeze-v28.json"
    ...
    if a.freeze:
        with FREEZE.open("x") as f:
            json.dump({"schema": "microduck.walking-evaluator-freeze/v1", "held_out": False,
                       "candidate": "V21 FINAL walker, V15 FINAL stander, V16 ramp and fixed V28 heading; exposed regression",
    ...
    with (a.output/"trajectory.jsonl").open("w") as stream:
        for timing in suite["timing_profiles"]:
            for case in suite["cases"]:
                ...
                world.heading_servo = MotionHeadingServo()
                ...
                try:
                            ...
                            try:
                                for i in range(round(suite["duration_s"]*50)):
                                    ...
                                    if writer and i%2 == 0:
                                        ...
                                        draw.text((10,8), f"V28 {role.upper()} | {name} | {row['session_time_s']:.2f}s", fill="white")
                    ...
                    world.close()
    result = {"schema": "microduck.walking-evaluation/v1", "acceptance_variant": "motion-heading-v28-flat-regression",
              ...
              "controller": "Fixed V16 command slew and V28 motion-established persistent-reference filtered IMU feedback; V21 FINAL walking / V15 FINAL standing; exact-zero ramped-command routing",
