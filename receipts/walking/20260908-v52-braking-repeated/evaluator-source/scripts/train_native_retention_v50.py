"""One bounded native PPO run learning downhill yaw with original standing fixed across complete sequences."""
import argparse
import copy
...
FREEZE=ROOT/'experiments/walking/native-retention-freeze-v50.json'
PARENTS={'walking':'walking-20260906-v21','standing':'standing-20260906-v15'}
EXTRA=['microduck/retained_walker_v50.py','microduck/native_sequence_env_v50.py',
       'scripts/train_native_retention_v50.py','scripts/probe_native_sequence_v50.py',
       'scripts/build_retention_replay_v50.py','scripts/analyze_components_v45.py',
       'experiments/walking/component_pairs_v45.py',
       'experiments/walking/RETENTION-CORRECTION-v50.md','experiments/walking/native_candidate_v50.py',
       'experiments/walking/retention-replay-v50/replay.npz','experiments/walking/retention-replay-v50/provenance.json',
       'scripts/evaluate_native_retention_flat_v50.py','scripts/evaluate_native_retention_endurance_v50.py',
       'scripts/evaluate_native_retention_surfaces_v50.py','scripts/evaluate_native_retention_fresh_v50.py',
       'scripts/verify_native_retention_v50.py','scripts/verify_retention_replay_v50.py','tests/test_retained_walker_v50.py']
...
def binding():
    ...
    return dict(schema='microduck.native-retention-freeze/v50',source_sha256=sources,
        warm_start=warm,seed=26090850,full_transitions=864000,smoke_transitions=2880)


def main():
    p=argparse.ArgumentParser();p.add_argument('--freeze',action='store_true');p.add_argument('--run-id')
    p.add_argument('--num-envs',type=int,default=48);p.add_argument('--iterations',type=int,default=750);a=p.parse_args()
    frozen=binding()
    if a.freeze:
        with FREEZE.open('x') as f:json.dump(frozen,f,indent=2);f.write('\n')
        return
    if not a.run_id or not a.run_id.replace('-','').isalnum() or (a.num_envs,a.iterations) not in [(24,5),(48,750)]:p.error('24x5 smoke or 48x750 full')
    if frozen!=json.loads(FREEZE.read_text()):raise ValueError('freeze drift')
    preflight=ROOT/'receipts/walking/20260908-v50-retention-conformance'
    for line in (preflight/'SHA256SUMS').read_text().splitlines():
        sha,name=line.split('  ',1)
        if digest(preflight/name)!=sha:raise ValueError('preflight drift')
    proof=json.loads((preflight/'probe.json').read_text())
    if not proof['passed'] or not proof['complete'] or len(proof['reports'])!=24 or proof['evaluator_freeze_sha256']!=digest(FREEZE):raise ValueError('conformance required')
    ...
    record=dict(schema='microduck.walking-training/v1',variant='native-retention-v50',proof_class='first_party_development',
    ...
    try:
        ...
        from microduck.native_sequence_env_v50 import NativeSequenceEnv
        torch.set_num_threads(1);torch.manual_seed(26090850);np.random.seed(26090850)
        cfg=copy.deepcopy(TRAIN_CFG);cfg.update(seed=26090850,run_name=a.run_id,save_interval=750)
        cfg['algorithm'].update(learning_rate=3e-5,schedule='fixed',entropy_coef=0.,gamma=.999)
        cfg['obs_groups']['critic']=['policy']
        cfg['actor']['class_name']='microduck.retained_walker_v50:RetainedWalkerActor'
        cfg['algorithm']['class_name']='microduck.retained_walker_v50:RetentionPPO'
        cfg['actor']['distribution_cfg']['std_range']=[.005,.15]
        record['train_cfg']=copy.deepcopy(cfg)
        env=NativeSequenceEnv(a.num_envs,26090850,out);record['env_cfg']=env.cfg
        ...
        data=np.load(ROOT/'experiments/walking/retention-replay-v50/replay.npz')
    ...
    for role in PARENTS:
        ...
        component={**record,'variant':'native-retention-'+role+'-v50','joint_checkpoint_sha256':record['checkpoint_sha256']}
