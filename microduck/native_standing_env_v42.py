...
class NativeStandingEnv:
    ...
    def __init__(self,num_envs,seed,output):
        ...
        self.cfg=dict(task='Native-Standing-Transitions-v42',backend='native-mujoco',actor_observations=61,
    ...
    def step(self,actions):
        ...
        for i,(w,action) in enumerate(zip(self.worlds,actions)):
            ...
            posture_cost=4*(max(0.,row['tilt_deg']-10.)/5.)**2 + 2*float(np.mean((np.maximum(np.abs(head)-.2,0.)/.15)**2))
            yaw_cost=.5*(row['yaw_rate_rad_s']/.3)**2
            rewards[i]=.02*(4*tracking-posture_cost-yaw_cost-2*max(0.,internal-.5)-10*(margin<.04)-.1*float(np.square(action-previous).sum()))-8*row['fell']
