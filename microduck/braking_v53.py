...
RUN=ROOT/'logs/braking-retention-20260908-v53'
...
def install(world):
    ...
    r=json.loads((RUN/'run.json').read_text());assert r['status']=='completed' and r['steps']==8000
    ...
    world.step_command=step;world.brake_v53=brake
    return brake
...
def retention_loss(prediction, target, positive_count=128):
    """Price average and worst-joint errors on verified zero-label states."""
    error=((prediction-target)/.03).square()
    positive=error[:positive_count].mean()
    zero=error[positive_count:]
    mean_zero=zero.mean()
    worst_zero=zero.max(dim=1).values.mean()
    return positive+8*mean_zero+8*worst_zero,(positive,mean_zero,worst_zero)
