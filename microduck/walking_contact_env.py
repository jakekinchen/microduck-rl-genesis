"""Versioned native constraint-default alignment, no mass or action change.

Genesis substitutes its own 10-ms default for omitted MJCF solver timing.
The frozen native MuJoCo model uses 20 ms. This version sets that declared
reference value; it does not fit contact parameters to a policy outcome.
"""
import torch
...
class MicroduckContactAlignedWalkingEnv(MicroduckGroundAlignedWalkingEnv):
    def __init__(self,num_envs,**kwargs):
        ...
        self.cfg.update(task="Walking-Posture-NativeConstraints-v6",walking_version="v6-native-constraints",
                        floor_contype=1,floor_conaffinity=1,constraint_timeconst_s=.02)
...
from .walking_tracking_env import MicroduckTrackingWalkingEnv


class MicroduckContactTrackingWalkingEnv(MicroduckTrackingWalkingEnv):
    _build_scene = MicroduckContactAlignedWalkingEnv._build_scene

    def __init__(self, num_envs, **kwargs):
        super().__init__(num_envs, **kwargs)
        self.cfg.update(task="Walking-Tracking-NativeConstraints-v6", walking_version="v6",
                        constraint_timeconst_s=.02)
