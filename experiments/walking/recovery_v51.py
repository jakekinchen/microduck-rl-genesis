"""V51 full-state branches and fixed recovery scoring in the native V30 world."""
import gzip
import hashlib
import json
...
FREEZE = ROOT/'experiments/walking/recovery-freeze-v51.json'
CAPTURE = ROOT/'receipts/walking/20260908-v51-recovery-capture'
COMPARE = ROOT/'receipts/walking/20260908-v51-recovery-comparisons'
SEARCH = ROOT/'receipts/walking/20260908-v51-recovery-search'
INPUTS = {'v50':'20260908-v50-retention-endurance'}
...
def source_data(key):
    ...
    sessions=[s for s in suite['sessions'] if s['id'].startswith('downhill')]
    ...
    for session in sessions:
        ...
        data.append(dict(key=key,folder=folder,suite=suite,session=session,rows=rows[session['id']],actions=actions,
                         capture_steps=[650,657] if session['id'].endswith('start-1') else [650]))
    return data
...
class TransientStander:
    """V48 is a diagnostic base for 2.5 s, then original V15 takes over."""
    def __init__(self,early,steady):self.early,self.steady,self.step=early,steady,0
    def infer(self,obs):return (self.early if self.step<125 else self.steady).infer(obs)


def anchors(world):
    from evaluator.core import OnnxPolicy
    config=world.core.config['inference']
    original=OnnxPolicy(ROOT/'receipts/walking/20260908-v50-retention-endurance/standing/policy.onnx',config)
    v48=OnnxPolicy(ROOT/'receipts/walking/20260908-v48-standing-retention-endurance/standing/policy.onnx',config)
    return {'original':original,'v48':v48,'transient':TransientStander(v48,original)}
...
class KnotPolicy:
    ...
    def __init__(self,base,knots):
        ...
        if self.knots.shape!=(5,14) or not np.isfinite(self.knots).all() or abs(self.knots).max()>1.500001:
            raise ValueError('invalid V51 knot parameters')
...
def rollout(world,state,source,start,anchor,knots=None,end=None,record=True):
    ...
    if not hasattr(world,'v51_base_policies'):
        from evaluator.core import OnnxPolicy
        world.v51_base_policies=anchors(world)
        world.v51_base_policies['walking']=OnnxPolicy(source['folder']/'policy.onnx',world.core.config['inference'])
    policies=world.v51_base_policies
    ...
    with record_self_loads(world) as loads:
        for i in range(start,end):
            ...
            policies['transient'].step=i-start
            if wrappers:
