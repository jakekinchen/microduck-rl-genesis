"""Frozen local BAM probes; independent fresh model/controller for every case."""
...
from metrics import classify, strict_action, dynamics_digest
...
def run(bank, case, output):
    ...
    save(output/'configuration.json',{'model_variant':case['model'],
        ...
        'gravity_m_s2':model.opt.gravity.tolist(),'initial_state_sha256':dynamics_digest(initial),
    ...
    try:
        for index,raw_action in enumerate(actions):
            ...
            for substep in range(4):
                ...
                row={'interval_start_s':t_before,'time_s':float(core.data.time),'action_index':index,'substep':substep,
                ...
                if stop:break
            if substep==3:timing['control_ns'].append(time.perf_counter_ns()-cycle_start)
    ...
    result.update(case=case,stop_reason=stop,initial_state_sha256=dynamics_digest(initial),
        ...
        contact_timing='applied solver loads for interval start/end; no extra mj_forward on physical data',
        passive_definition=bank['passive_definition'] if case['mode']=='passive' else None)
    if case['model']=='v11' and case['mode']=='replay':
        golden=[]
