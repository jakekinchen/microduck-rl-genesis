"""V64 classification: V63 physical thresholds with explicit integration dt."""
import hashlib
import numpy as np
from contact_v64 import load_gate
...
def classify(rows, case, thresholds, timing):
    dt=case['dt_s']
    expected = round(case['duration_s']/dt)
    failures = []
    ...
    if not np.isfinite(values).all() or any(abs(r['time_s']-(i+1)*dt)>1e-8 for i,r in enumerate(rows)):
        failures.append('invalid_or_unordered_physics_evidence')
    load = load_gate(rows,case['duration_s'],dt)
    ...
    tail = rows[round((case['duration_s']-1.)/dt):]
    if case['mode'] == 'passive':
        ...
        if len(tail) != round(1./dt) or max(r['speed_m_s'] for r in tail) > thresholds['passive_final_speed_m_s']:
            failures.append('passive_not_settled_linear')
        if len(tail) != round(1./dt) or max(r['angular_speed_rad_s'] for r in tail) > thresholds['passive_final_angular_speed_rad_s']:
    ...
    perf['provisional_50hz_budget_passed'] = bool(complete and len(cycle_ns)==round(case['duration_s']/.02)
