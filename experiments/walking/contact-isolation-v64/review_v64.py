...
def verify_case(case,bank,source,home):
    ...
    with np.load(path/'dynamics.npz') as archive,gzip.open(path/'physics.jsonl.gz','rt') as stream:
...
def main():
    ...
    bank=load(BASE/('bank-r2.json' if (BASE/'bank-r2.json').exists() else 'bank.json'));source=np.load(ROOT/bank['actions_file'])
    ...
    save(BASE/(args.phase+'-review-r2.json'),report)
