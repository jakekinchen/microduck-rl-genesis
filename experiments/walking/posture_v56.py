"""Fixed identities and safe new-data placement for the V56 standing-posture comparison."""
import hashlib
import json
...
def run_name(recipe, smoke=False):
    ...
    return 'posture-native-20260916-v56-' + recipe + ('-smoke' if smoke else '')
...
def candidate(recipe):
    ...
    if (record['status'] != 'completed' or record['recipe'] != recipe
        ...
        raise ValueError('exact bounded V56 final required')
