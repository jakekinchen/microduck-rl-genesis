"""Fixed identities and safe new-data placement for the V54 comparison."""
import hashlib
import json
...
def run_name(recipe, smoke=False):
    ...
    return 'recipe-native-20260909-v54-' + recipe + ('-smoke' if smoke else '')
...
def candidate(recipe):
    ...
    if (record['status'] != 'completed' or record['recipe'] != recipe
            or record['checkpoint'] != 'model_1499.pt'
            or record['new_transitions'] != 1_728_000
            or record['source_sha256'] != frozen['source_sha256']):
        raise ValueError('exact bounded V54 final required')
