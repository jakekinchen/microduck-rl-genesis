"""V62 unit-stable sampled material validation, independent of compilation.

V59 sampling and acceptance rules are retained. Trimesh proximity and ray
queries run in millimetres because its absolute tolerances are not scale
invariant on small CAD triangles. All stored geometry and metrics stay metres.
The original V59 module and its negative results remain unchanged.
...
def _validate_one(
    ...
    try:
        ...
        if missing.any():
            ...
            for part in parts:
                query_part = part.copy()
                query_part.apply_scale(1000.0)
                distances = np.minimum(distances, trimesh.proximity.closest_point(
                    query_part, source_points[missing] * 1000.0)[1] / 1000.0)
        ...
        query_source = source.copy()
        query_source.apply_scale(1000.0)
        excess_distance = np.zeros(len(proxy_points))
        for start in range(0, len(proxy_points), 512):
            points_mm = proxy_points[start : start + 512] * 1000.0
            outside = ~query_source.contains(points_mm)
            ids = np.flatnonzero(outside) + start
            if len(ids):
                excess_distance[ids] = trimesh.proximity.closest_point(
                    query_source, points_mm[outside])[1] / 1000.0
...
def validate_meshes(
    ...
    report: dict[str, Any] = {
        ...
        "settings": {
            "query_units": "millimetres; output metres", "validator_revision": "v62-unit-stable",
