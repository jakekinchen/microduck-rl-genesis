...
def partition_source(source, clipped, config, distance_mesh=None):
    ...
    while pending:
        ...
        if error <= config['refinement_target_m']:
            ...
            nearest, _, triangle = trimesh.proximity.closest_point(mm, points[distances.argmax()][None]*1000.0)
            normal = np.asarray(mm.face_normals[int(triangle[0])])
            # Put the cut inside the witnessed empty gap, away from an exactly
            # coincident source face that can create degenerate Boolean halves.
            gap_midpoint = (nearest[0]/1000.0 + points[distances.argmax()])/2
            offset = float(np.dot(normal, gap_midpoint))
            method = 'source_normal_gap_midplane'
            ...
            if (len(usable) != 2 or min(c.volume() for c in usable) < solid.volume()*1e-6
                    or any(len(from_manifold(c).vertices) < 4 for c in usable)):
