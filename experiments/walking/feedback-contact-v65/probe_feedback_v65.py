"""V65 frozen replay with captured BAM force inputs; V64 geometry and physics unchanged."""
...
sys.path.insert(1, str(ROOT/'experiments/walking/contact-isolation-v64'))
from feedback_v65 import ForceSnapshot, ForceHistory, update_with_snapshot
...
def run(bank, case, output):
    ...
    if case['mode'] != 'replay' or case['intervention'] != 'none':
        raise ValueError('V65 only isolates replay force-input age')
    assert int(mujoco.mjtConstraint.mjCNSTR_FRICTION_DOF) == 1
    ...
    assert core.joint_ids.tolist()==bank['joint_ids']
    assert core.dof_indices.tolist()==bank['dof_indices']
    ...
    written=0
    initial_force=ForceSnapshot.capture(core.data,-1,0.)
    history=ForceHistory(microsteps,initial_force)
    force_trace=gzip.open(output/'solver-forces.jsonl.gz','wt')
    input_trace=gzip.open(output/'bam-inputs.jsonl.gz','wt')
    force_trace.write(json.dumps(initial_force.record(),allow_nan=False)+'\n')
    inputs=[]
    try:
        for index,raw_action in enumerate(actions):
            ...
            for substep in range(4):
                ...
                if case['mode']=='passive':
                    ...
                    tick=index*4+substep
                    selected=history.select(tick,case['feedback'],float(core.data.time))
                    inputs.append(update_with_snapshot(core.controller,core.data,selected,tick))
                    input_trace.write(json.dumps(inputs[-1],allow_nan=False)+'\n')
                ...
                for integration_substep in range(microsteps):
                    ...
                    snapshot=ForceSnapshot.capture(core.data,len(rows),t_before)
                    history.append(snapshot)
                    force_trace.write(json.dumps(snapshot.record(),allow_nan=False)+'\n')
    ...
    result['feedback']={'mode':case['feedback'],'bam_updates':len(inputs),
        'post_bootstrap_age_s':dt if case['feedback']=='native' else .005,
        'solver_fields_preserved':all(r['physical_solver_fields_unchanged'] for r in inputs),
        'boundary':'saved solver inputs only; live qpos/qvel and clocks retained; no extra physical mj_forward'}
