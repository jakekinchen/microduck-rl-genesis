# Retained-controller runtime contract — v2 correction, September 15

This version supersedes the phase ordering in v1; v1 and its original closure
remain unchanged. Applied contact loads are sampled after `mj_step`, and the
V11 model is an eleven-collider reference, not full-CAD physical coverage.
...
4. **BAM and contact physics.** Reuse the existing eleven-collider contact-v11 reference physical
   arrays for exact legacy comparison and pinned BAM `62bd8ce12154340be97e06f7f41a0ca8f116d967`. Every 20 ms
   ...
   record the commanded torque with its pre-step timestamp, then mj_step,
   and only then read actual applied contact loads with their post-step timestamp.
   Do not call an extra forward/step on physical data to obtain contact loads. Nominal motor FIFO is four physics
   ticks; declared 30 ms case is six. Initial FIFO values are HOME. Static XML
   position gains cannot substitute for BAM. For a legacy conformance comparison keep masks, exclusions,
   armature and friction identical; read internal loads without changing dynamics. New recovery admission requires
   the reconciled full-CAD model and its actual-surface/contact audit; V11
   conformance alone is insufficient. Preserve that changed-model result
   separately from the V11 baseline.
